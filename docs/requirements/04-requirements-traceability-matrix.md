...
| FR-9110 | Seed/scale immutable per save, new-game-only (target — no write path exists until FEAT-1100) | — | ADR-0010 | ADR-0010 | `asm_game.py` (`generate_world` reads only, never writes, `SEED`/`WORLD_SCALE`) | FS-102 | IP-1020 | UNASSIGNED — vacuously true today (nothing writes `SEED`/`WORLD_SCALE` yet); full negative-test sweep is `IP-1040`'s T13.e |
