...
| [IP-1126](packages/IP-1126-infinite-mode-combat-mob-movement.md) | Infinite Mode Combat: Mob Movement | `08-code-implementation` | **READY** | IP-1121 (VERIFIED) | **AUTHORIZED (G3, user go-ahead, 2026-07-19 — "Authorized but don't iterate now")** | Planned 2026-07-19 (`07-implementation-planning`). Per-frame mob movement toward the player: dominant-axis single-step, adjustable `MOB_MOVE_INTERVAL`/`MOB_MOVE_STEP` (starting values 8 frames / 1 pixel), holds still once coincident with the player (resolves `FS-112` Open Question 4). New WRAM `MOB_MOVE_TIMER` (`0xC6DE`). New suite `T35` (9 checks incl. a live PyBoy drive). Sole dependency `IP-1121` already `VERIFIED` — `READY` and authorized; build deliberately deferred to a later session/turn per the user's own explicit request. |
