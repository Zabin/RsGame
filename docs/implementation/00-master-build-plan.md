...
| [IP-1127](packages/IP-1127-infinite-mode-combat-post-contact-protection.md) | Infinite Mode Combat: Post-Contact Player Protection | `08-code-implementation` | **NOT STARTED** (structurally unblocked 2026-07-19 — `IP-1123` reached `VERIFIED`; still gated on G3) | IP-1121 (VERIFIED), IP-1123 (VERIFIED) | **NOT AUTHORIZED** — new scope beyond the original "Yes, build all six" go-ahead; `FR-11410` baselined 2026-07-19, the user's own direct decision on `BL-0158`'s live-drive finding (invincibility + knockback + per-mob cooldown, combined). | Planned 2026-07-19 (`07-implementation-planning`). Extends `inf_mob_contact_check` (`IP-1123`) with a per-mob cooldown bit (new parallel table `MOB_CONTACT_FLAGS`, `0xC6E0` — deliberately not a `MOB_DATA` stride widening), an invincibility countdown (`PLAYER_INVINCIBLE`, `0xC6DF`), and a knockback push (clamped to the existing position-bounds precedent). New suite `T36` (10 checks incl. the exact `BL-0158` repro and a live PyBoy drive). `BLOCKED` on `IP-1123`'s own pending `09` Pass 2 — building against unverified code would compound risk. **WRAM collision resolved against `IP-1128` (2026-07-19):** `IP-1128` shipped first and claimed `0xC6DF`–`0xC6E1` for real (`PLAYER_FACING_X`/`Y`/`PROJ_STEP_Y`) — this package's own `PLAYER_INVINCIBLE`/`MOB_CONTACT_FLAGS` addresses must be re-derived to the next free byte(s) past `0xC6E1` when this package is next picked up for implementation; its own §6 Files-to-Modify is now stale on this point and needs a drift check at that time. |
...
and is now `COMPLETE`, own `09-package-verification` pass owed. `IP-1127` branches off `IP-1121`
and `IP-1123` — structurally unblocked 2026-07-19 (`IP-1123` reached `VERIFIED`, Pass 2), but
still `NOT AUTHORIZED` (own G3 owed, new scope beyond "build all six"), independent of `IP-1126`'s
own status. Neither extends the original critical path (`IP-1124` does not depend on either).
