...
> three packages authorized 2026-07-11 (user G3, `BL-0062`); `IP-9070` reached `COMPLETE` the
> same session (implementer independence required before it can reach `VERIFIED`).** Owned by
...
| [IP-9050](packages/IP-9050-generated-world-navigation-fix.md) | Generated-world navigation fix (BL-0047) | `08-code-implementation` | **READY** | IP-9070 (COMPLETE — hard prerequisite), IP-1020 (VERIFIED), IP-1030 (VERIFIED) | **YES — explicit user G3, 2026-07-11 (BL-0062)** | Regeneralizes `check_zone_transition` to read `REGION_GRAPH`'s neighbor bytes instead of the hardcoded fixed-3×3 `CUR_ZONE` arithmetic still shipped today — the missing *navigate* verb `IP-1020`/`IP-1030` never packaged. **Critical** — root cause of "the map remains a 3×3 grid regardless of scale/seed." `IP-9070` reached `COMPLETE` 2026-07-11, unblocking this package (correctness-ordering dependency, not scheduling convenience — this fix makes `CUR_ZONE > 8` reachable, `IP-9070` is what makes that safe). |
| [IP-9060](packages/IP-9060-main-menu-cursor-fix.md) | Main menu cursor fix (BL-0048) | `08-code-implementation` | **READY** | IP-1040 (VERIFIED) | **YES — explicit user G3, 2026-07-11 (BL-0062)** | Fixes `check_save_valid`'s `MM_CURSOR`-reset side effect, which clobbers the player's own MAIN MENU UP/DOWN toggle on every redraw — "new game" is currently unreachable whenever a save exists. **High.** Independent of `IP-9050`/`IP-9070` — parallel-eligible. |
...
graph TD
    ...
    IP9050["IP-9050 generated-world<br/>navigation fix<br/>(remediation, READY)"]
...
- **Critical path: IP-9070 → IP-9050** (2 packages) — `IP-9070` widens/relocates every
  ...
  scheduling convenience (see `IP-9050`'s own Dependencies field). **`IP-9070` reached `COMPLETE`
  2026-07-11, unblocking `IP-9050` to `READY`.**
