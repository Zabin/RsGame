...
| [IP-1122](packages/IP-1122-infinite-mode-combat-weapon-fire-and-hit-resolution.md) | Infinite Mode Combat: Weapon Fire & Hit Resolution | `08-code-implementation` | **READY** | IP-1121 (VERIFIED), IP-1125 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | New `PROJ_*`/`WEAPON_TIER` WRAM (`0xC6D5`–`0xC6D9`); A-button fire input (confirmed free); reuses `check_collisions`' hitbox technique. **Found a genuine requirements gap during planning**: `WEAPON_TIER`'s own treasure-funded upgrade mechanism has no baselined FR (`FR-11500` covers healing only) — harvested to the backlog, routed to `04-requirements-engineering` (`BL-0147`). All dependencies now `VERIFIED` — eligible for the next `08-code-implementation` pick. |
