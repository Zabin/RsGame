...
| [IP-1106](packages/IP-1106-infinite-mode-nine-identity-value-widening.md) | Infinite Mode nine-identity value-range widening | `08-code-implementation` | **VERIFIED** ([VR-1106](verification/VR-1106-infinite-mode-nine-identity-value-widening.md), fresh session, 2026-07-17) | IP-1105 (VERIFIED), IP-1022 (VERIFIED), IP-1033 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `materialize_region`'s draw `%5`→`%9`, extends `inf_treasure_pos` to nine entries. **Implemented 2026-07-17** (all three dependencies `VERIFIED` first, per its own hard sequencing precondition): `worldgen.py` `% 5`→`% 9`; `asm_game.py`'s `inf_mod5` widened in place to `inf_mod9` (single call site — the biome draw — confirmed by direct grep); `inf_treasure_pos` extended to nine `(x,y)` entries copied verbatim from `IP-1033`'s `ZONE_COLLECTS` type-2 entries per the table's established duplication convention. ROM: 32414/32768 (354 headroom — +256 over `IP-1022`'s figure: the 8 new table bytes pushed code end across a 256-byte alignment boundary, shifting the data blocks; no overlap, build asserts clean). **313/313 suite passes** including extended `T26.a0` (nine-entry drift guard), new `T26.h` (value-range coverage — all nine values reached in live SM83 output across T22's corpus, not the oracle, closing the same vacuity risk `T12.d`'s IP-1022 lesson named), and new `T26.i` (dispatch-integration — biome-ids 5-8 render their own family screen AND spawn their own treasure position from the Infinite Mode path specifically). **Independently verified 2026-07-17 (`VR-1106`, fresh session): 321/321 re-run on current tree head (includes `IP-1111`/`IP-9160`/`IP-9150` layered on top), ROM 32768/32768 valid build, `inf_treasure_pos`'s nine entries re-derived from source and confirmed matching `ZONE_COLLECTS`'s type-2 entries exactly, `IP-1105`/`IP-1022` confirmed byte-for-byte unchanged by direct diff, diff scope confirmed exactly within §6.** No findings. |
| [IP-9160](packages/IP-9160-procedural-screen-zone-name-restoration.md) | Procedural-screen zone-name restoration (`BL-0138`, content-review Medium) | `08-content-authoring` | **VERIFIED** ([VR-9160](verification/VR-9160-procedural-screen-zone-name-restoration.md), fresh session, 2026-07-17) | IP-1022 (VERIFIED), IP-1033 (VERIFIED) | **AUTHORIZED (G3, user explicit go-ahead, 2026-07-17)** | The four `ADR-0020` procedural screens never write the row-0 zone name their Python oracles ship — the previous screen's name persists ([content-review finding 1](../reviews/content-review-nine-biome-family-delta.md)). Fix is pure data: append each screen's name-region row-0 cells (mechanically derived from its own `*_screen()` oracle) to the existing `*_LANDMARKS` lists — `apply_landmark_overlay` already writes row 0 unfiltered, `build_rom.py`'s emission is generic (both direct-read confirmed). Tests: `T13.e` exclusion narrowed to the live digit cells (row 0's static region enters the parity check), new `T13.g` stale-name regression both directions. ≈128 data bytes vs. 354 headroom. **Independently verified 2026-07-17 (`VR-9160`, fresh session): 321/321 re-run, ROM 32768/32768 valid build, own independent live drive (fresh boot, Forest→Village force, own screenshot) confirms the name-region VRAM bytes byte-for-byte match `village_screen()`'s oracle.** No findings. |
| [IP-9150](packages/IP-9150-tile-data-padding-trim.md) | Tile-data padding trim (ROM-budget hygiene, `BL-0134`'s safe mechanical recovery) | `08-code-implementation` | **VERIFIED** ([VR-9150](verification/VR-9150-tile-data-padding-trim.md), fresh session, 2026-07-17) | None | **AUTHORIZED (G3, user explicit go-ahead, 2026-07-17)** | `tiles.py`'s `build_tile_data()` allocates a fixed 256-tile (4,096-byte) array; only 182 slots (indices 0–181, highest is `TL_TORCH=0xB5`) are ever used. Trimming to 184 tiles (2,944 bytes) + correspondingly reducing the boot-time tile-copy DMA count (`asm_game.py:359`) recovers 1,152 bytes with zero behavior change. Independent of `IP-1022` — good hygiene, no longer strictly required to unblock it now that `ADR-0020`'s procedural-fill approach is expected to close the full gap on its own. **Independently verified 2026-07-17 (`VR-9150`, fresh session): 321/321 re-run, ROM 32768/32768 valid build, shared `TILE_DATA_TILES` constant confirmed by direct code read on both sites, highest-used `TL_*` re-derived as 181 independently, exact 1,152-byte recovery confirmed.** No findings. |

**Critical-path chain within this delta:** `IP-1033`/`IP-1105` (parallel-eligible roots, both
`VERIFIED`) → `IP-1022` (`VERIFIED`, `VR-1022` 2026-07-17) → `IP-1106` (`VERIFIED`, `VR-1106`
2026-07-17) — this arc's own critical path is now fully `VERIFIED` end to end.
...
**Authorization state (all six packages, current):** `IP-1105`/`IP-1033`/`IP-1110`/`IP-1022`/
`IP-1106`/`IP-1111`/`IP-9160`/`IP-9150` **all `VERIFIED`** — every package in this delta's full
eight-package set is now `VERIFIED`; a `10-integration-review` pass across the set is the natural
next step. All eight (six original plus the two remediation packages) were authorized
together ("Build all six," 2026-07-16; remediations
user-authorized 2026-07-17), so nothing in this tranche is gated on G3.
...
| [IP-1111](packages/IP-1111-biome-family-sub-theme-playback-selection.md) | Biome-Family Sub-Theme Playback Selection | `08-code-implementation` | **VERIFIED** ([VR-1111](verification/VR-1111-biome-family-sub-theme-playback-selection.md), fresh session, 2026-07-17) | IP-1110 (VERIFIED), IP-1022 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Hooks into `do_screen_redraw`'s per-state dispatch (default reset to main theme) and `dsr_p_dispatch`'s per-identity cascade (override to matching sub-theme) — resolves `FS-111`'s Open Question 1. Also fixes `music_tick`'s hardcoded loop-restart target (new `MUSIC_BASE_LO`/`MUSIC_BASE_HI`), a correctness gap this planning pass found, not named in `FS-111`. **Re-planned 2026-07-17** to consume `IP-1110`'s shipped `music_table` (see the package's revision history). **Implemented 2026-07-17**: `music_select` subroutine (preserves A for the cascade; reads `music_table` via the new `music_tbl` patch key, `zc_table` pattern), unconditional main-theme default at every `do_screen_redraw` entry, single `CALL` at `dsr_p_dispatch`'s entry covering all nine identities on both mode paths, `music_tick` loop-restart retarget to the new `MUSIC_BASE_LO`/`MUSIC_BASE_HI` (`0xC6B3`/`0xC6B4`, GDS-07 updated), `mus_reset` fully retired (grep-confirmed no live references), boot-init extended defensively. ROM 32414/32768 (total unchanged — new code fit inside the existing alignment slack). **319/319 suite passes** (new `T28.a0`/`a`–`e`: static table audit; selection all nine ids finite path + Desert via the Infinite Mode window path; eleven-state main-theme fallback; loop-restart from the sub-theme's own start; one-frame repoint timing). One test-authoring lesson recorded: `MUSIC_PTR` is a live cursor (advances 3 bytes/note) — assertions pin `MUSIC_BASE` + in-track range, not exact PTR equality. **Independently verified 2026-07-17 (`VR-1111`, fresh session): 321/321 re-run on current tree head, ROM 32768/32768 valid build, `music_select`'s A-preservation confirmed by direct code read (`PUSH_AF`/`POP_AF` bracket), `mus_reset` confirmed zero live references (grep, comments only), `dsr_p_dispatch`'s cascade confirmed unchanged beyond the one inserted `CALL`.** No findings. |

**Critical-path chain within this delta:** `IP-1110` (`VERIFIED`) → `IP-1111` (`VERIFIED`, `VR-1111`
2026-07-17) — this arc's own critical path is now fully `VERIFIED` end to end.
...
| Package | Title | Owner (08 peer) | Status | Depends on | Authorized? | Notes |
|---|---|---|---|---|---|---|
| [IP-9170](packages/IP-9170-hud-carrot-target-digit-fix.md) | HUD carrot-target digit fix | `08-code-implementation` | **VERIFIED** ([VR-9170](verification/VR-9170-hud-carrot-target-digit-fix.md)) | None | **AUTHORIZED (G3, user explicit go-ahead, 2026-07-17)** | `update_status_disp` gains a `GAME_MODE`-gated write of `WORLD_SCALE` to the HUD's row-0/col-4 target digit (finite mode only, col 4 = `0x9804`). Infinite Mode's own col-4 cell deliberately untouched. **Implemented 2026-07-17** — one `GAME_MODE`-gated conditional write inserted immediately after the existing carrot-count write in `update_status_disp`; no other line changed. Fixed a real regression this package's own change exposed in an existing check: `T13.e`'s row-0 dynamic-cell exclusion set didn't yet know col 4 was now live in finite mode (it compares screens rendered via the suite's own finite-mode fixture) — extended `_ROW0_DYNAMIC_RC` to include `(0, 4)`. **324/324 suite passes** (new `T8.10c`/`T8.10d`: HUD target digit tracks forced `WORLD_SCALE` values 5 and 7; new `T8.10e`: Infinite Mode's own col-4 cell confirmed unaffected by `WORLD_SCALE` changes once `GAME_MODE=1`). ROM 31390/32768 (unchanged — fit inside existing alignment slack). **Independently verified 2026-07-18** (`VR-9170`, fresh session, 330/330 suite, live-driven at non-default `WORLD_SCALE=6`/`8`) — `VERIFIED`, no findings. |
| [IP-9180](packages/IP-9180-infinite-mode-hud-treasure-count-digit.md) | Infinite Mode HUD target-digit convention (`BL-0144`) | `08-code-implementation` | **VERIFIED** ([VR-9180](verification/VR-9180-infinite-mode-hud-treasure-count-digit.md)) | IP-9170 (VERIFIED) | **AUTHORIZED (G3, user explicit go-ahead, 2026-07-17)** | Extends `IP-9170`'s own `GAME_MODE` branch in `update_status_disp`: the Infinite Mode half writes `RUNNING_TREASURE_COUNT`'s low byte reduced mod 10 to the same `0x9804` cell (user decision, 2026-07-17). Single-digit-cell constraint named as an accepted scoping choice, not a defect. **Implemented 2026-07-17** — `usd_infinite_target` branch added (repeated-subtraction mod-10, mirroring `inf_mod9`'s established technique, bound 10). **326/326 suite passes** (new `T8.10e`/`T8.10f`: digit tracks forced `RUNNING_TREASURE_COUNT` 7 and 13→3, confirming the mod-10 wrap; new `T8.10g`: finite mode's own `WORLD_SCALE` display confirmed unaffected). ROM 31390/32768 (unchanged — fit inside existing alignment slack). **Independently verified 2026-07-18** (`VR-9180`, fresh session, 330/330 suite, live-driven at non-default `RUNNING_TREASURE_COUNT=9`/`21`) — `VERIFIED`, no findings. |

No critical-path interaction beyond `IP-9170`→`IP-9180`'s own sequencing — independent of every other package in the tree.
...
**and AUTHORIZED** (G3, user "Yes, build all six," 2026-07-17).

| Package | Title | Owner (08 peer) | Status | Depends on | Authorized? | Notes |
|---|---|---|---|---|---|---|
| [IP-1125](packages/IP-1125-combat-sprite-content.md) | Combat Sub-Mode: Mob & Projectile Sprite Content | `08-content-authoring` | **VERIFIED** ([VR-1125](verification/VR-1125-combat-sprite-content.md); `09-content-review` still owed) | None | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | **Implemented 2026-07-17**: `TL_MOB`/`TL_MOB_BOT`/`TL_PROJECTILE`/`TL_PROJECTILE_BOT` (`0x0A`–`0x0D`) registered in `build_tile_data()`; OBJ palettes 4/5 repurposed from placeholder "unused/white" to real colors (purple mob, orange/yellow projectile) — zero new palette slots (`build_rom.py`'s `OBJ_PALETTES` stays 8 fixed entries). New suite `T34` (4 checks: index/registration/distinctness/palette-budget). **330/330 suite passes**, ROM unchanged 31390/32768. Live-driven via PyBoy (forced `OAM_BUF` entries, one-frame DMA settle) — both sprites confirmed rendering and visually distinct. Unblocks `IP-1121`/`IP-1122` to `READY` (their own remaining dependency, `IP-1101`/`IP-1102`, already `VERIFIED`). **Independently verified 2026-07-18** (`VR-1125`, fresh session, 330/330 suite, own live drive/screenshot) — `VERIFIED`. One Low finding: `IP-1125` §6 omitted `build_rom.py` from its declared file set despite necessarily/correctly touching it. `09-content-review` (qualitative judgment on the sprite art) still owed. |
| [IP-1121](packages/IP-1121-infinite-mode-combat-mob-materialization-and-rendering.md) | Infinite Mode Combat: Mob Materialization, Rendering & Defeat | `08-code-implementation` | **VERIFIED** ([VR-1121](verification/VR-1121-infinite-mode-combat-mob-materialization-and-rendering.md)) | IP-1101/IP-1102 (VERIFIED), IP-1125 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | **Implemented 2026-07-18**: `COMBAT_MODE`/`MOB_COUNT`/`MOB_DATA` (`0xC6B5`–`0xC6D4`, boot-cleared) defined; `inf_materialize_mobs` hooked into `inf_ensure_window`'s existing center-cell recompute (own decorrelated reseed, XOR-salted column input — avoids the exact biome-correlation defect `worldgen.py`'s own `materialize_region` docstring already documents catching once); `inf_mob_render` hooked into `update_oam` (required changing `COLL_COUNT==0`'s early `RET_Z` to a `JR_Z` past the collectible loop, so mobs still render on a treasure-less region — a necessary, minimal fix, not scope creep); `inf_mob_defeat` defined/exposed (no call site yet, `IP-1122`'s own scope). **Design deviation from the package's own "next free slot" phrasing, named explicitly**: writes all 6 candidate slots at fixed 1:1 indices (not front-compaction) — functionally equivalent, avoids the compaction-order fragility a genuine defeat-driven compaction would introduce, and is what let a real defect surface and get fixed during implementation: `inf_mob_render` was originally gated on `MOB_COUNT` (mirroring `COLL_COUNT`), but `MOB_COUNT` is decremented by `inf_mob_defeat` — a `MOB_COUNT`-gated render would stop scanning before a later still-active slot and never re-hide a defeated slot's own stale OAM entry (caught by `T29.e`, fixed by gating on `COMBAT_MODE` instead, always scanning all 6 slots). **336/336 suite passes** (new suite `T29`, 6 checks: `T29.a` determinism, `T29.b` oracle/SM83 lockstep against new `worldgen.materialize_mobs`, `T29.c` `COMBAT_MODE`-off no-op, `T29.d` mob-count ceiling, `T29.e` defeat/no-stale-OAM, `T29.f` OAM budget static audit). ROM 31646/32768 (+256 bytes, within the 1,378-byte headroom). Live-driven via PyBoy: forced `COMBAT_MODE`/`MOB_DATA` state during real `PLAYING`, confirmed mobs render correctly through the real per-frame `update_oam` call path (not just the direct-invoke test harness) — screenshot confirms three distinct purple mob sprites on-screen. `FR-11200` → Implemented; RTM row updated; `FS-112`/`GDS-07` §7i documentation updated. **Independently verified 2026-07-18** (`VR-1121`, fresh session, 336/336 suite, rebuild byte-identical to checked-in ROM) — all DoD/checklist items confirmed by direct code read, no scope excursion. Own independent live drive at a non-fixture `(SEED=314159, row=12, col=-7)` via the real production call chain (`inf_ensure_window`→`update_oam`, not the test harness's direct-invoke helpers) — oracle-exact mob materialization and correct OAM rendering confirmed. `VERIFIED`, no findings. Unblocks `IP-1122`/`IP-1123`/`IP-1120` to `READY`. |
| [IP-1122](packages/IP-1122-infinite-mode-combat-weapon-fire-and-hit-resolution.md) | Infinite Mode Combat: Weapon Fire & Hit Resolution | `08-code-implementation` | **VERIFIED** ([VR-1122](verification/VR-1122-infinite-mode-combat-weapon-fire-and-hit-resolution.md)) | IP-1121 (VERIFIED), IP-1125 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | **Implemented 2026-07-18**: `PROJ_ACTIVE`/`PROJ_X`/`PROJ_Y`/`PROJ_DIR`/`WEAPON_TIER` (`0xC6D5`–`0xC6D9`, `PROJ_ACTIVE` boot-cleared, `WEAPON_TIER` boot-initialized to 1) defined; `handle_play_input` gains a `COMBAT_MODE`-gated A-button fire branch (spawns a projectile at the player's own position/facing if none active); `inf_projectile_update` hooked into `st_playing`'s per-frame chain (a no-op unless `COMBAT_MODE`/`PROJ_ACTIVE`); `inf_projectile_hittest` reuses `check_collisions`' own asymmetric point-in-box technique verbatim (unmodified) against `MOB_DATA`, subtracting `WEAPON_TIER` from a hit mob's health (floored at 0, calling `IP-1121`'s `inf_mob_defeat` at zero), always stopping the projectile on any hit; `update_oam` extended with a `PROJ_ACTIVE`-gated OAM entry (`TL_PROJECTILE`, palette 5). **Named deviation, documented explicitly**: `PROJ_DIR` mirrors `PLAYER_DIR`'s own real 2-value encoding (0=right/1=left), not the "0-3" range this package's own §6 assumed — direct code read confirms `PLAYER_DIR` is written only by RIGHT/LEFT, never UP/DOWN; `FR-11300`'s own Notes delegate the exact facing-direction mechanism to `06`/`08` discretion, so the projectile moves horizontally only (harvested as a Low doc-accuracy finding, `BL-0151`). **342/342 suite passes** (new suite `T30`, 6 checks: `T30.a` fire spawns, `T30.b` no double-fire, `T30.c` hit resolution + defeat, `T30.c2` non-lethal-hit spot check, `T30.d` miss/terminal boundary, `T30.e` `COMBAT_MODE`-off no-op). ROM 31646/32768 (net +0 bytes used — the ~194 bytes of new code fit entirely inside existing 0x100-alignment padding slack, confirmed by direct before/after `rom.pos` measurement). **Rebuild discipline note**: this run's first full-suite attempt hung (100% CPU, no progress) because the checked-in `BunnyQuest.gbc` had not yet been rebuilt from the edited source while the test harness re-resolves labels fresh from source each run — PC-hijack tests jumped to addresses valid in the new source but not in the stale binary; `python3 build_rom.py` (default output path) resolved it, and the DoD's own "ROM builds" gate is what would have caught this even without the diagnosis. `FR-11300` → Implemented; RTM row updated; `FS-112`/`GDS-07` §7j documentation added. `WEAPON_TIER`'s own funding-mechanism gap remains open, unresolved by this package (`BL-0147`, unchanged). **Independently verified 2026-07-19** ([VR-1122](verification/VR-1122-infinite-mode-combat-weapon-fire-and-hit-resolution.md), fresh session, 363/363 suite, own live PyBoy drive through the real per-frame production call chain) — `VERIFIED`, no findings. |
| [IP-1123](packages/IP-1123-infinite-mode-combat-player-health-and-economy.md) | Infinite Mode Combat: Player Health, Setback & Healing Economy | `08-code-implementation` | **VERIFIED** ([VR-1123](verification/VR-1123-infinite-mode-combat-player-health-and-economy.md): Pass 1 `RETURNED`; [Pass 2](verification/VR-1123-infinite-mode-combat-player-health-and-economy-pass2.md): `VERIFIED`, 2026-07-19) | IP-1121 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | **Implemented 2026-07-18**: `PLAYER_HEALTH`/`COMBAT_ENTRY_X`/`COMBAT_ENTRY_Y` (`0xC6DA`–`0xC6DC`, `PLAYER_HEALTH` boot-initialized to 3) defined; `inf_mob_contact_check` (hooked into `st_playing`, reuses `check_collisions`' own technique verbatim against `PLAYER_X`/`Y`, decrements `PLAYER_HEALTH` on contact, calls `inf_health_setback` at zero); `inf_health_setback` (restores max health, repositions to `COMBAT_ENTRY_X`/`Y`, never writes `GAMESTATE`); `inf_record_combat_entry` (hooked into all six `inf_ensure_window` call sites: initial entry, all four `czt_infinite` transition branches, post-load restore); `inf_heal_spend` (decrements `RUNNING_TREASURE_COUNT` directly, heals capped at max — defined/exposed, **not called from anywhere**, per `BL-0148`'s own still-unresolved input-binding gap); `inf_health_hud_draw` (row-1 heart cells, VRAM `0x9820`–`0x9822`, hooked into `update_status_disp`). Also set `SCORE_DIRTY` wherever `PLAYER_HEALTH`/`RUNNING_TREASURE_COUNT` change (`inf_mob_contact_check`/`inf_heal_spend`) so the HUD redraws promptly — a small correctness addition within this package's own "the HUD reflects it" DoD, not scope creep. **349/349 suite passes** (new suite `T31`, 7 checks: `T31.a` contact damage, `T31.b` HUD full/empty pattern across 0-3, `T31.c` zero-health setback, `T31.d` heal-spend decrement/heal, `T31.d2` heal-cap spot check, `T31.e` zero-treasure no-op, `T31.f` `COMBAT_MODE`-off no-op). ROM 31902/32768 (headroom 866 bytes, down from 1,122 — the ~228 bytes of new code crossed a `0x100` alignment boundary this time, unlike `IP-1122`'s own zero-net-growth case). `FR-11400`/`FR-11500` → Implemented; RTM rows updated; `FS-112`/`GDS-07` §7k documentation added; §19 Open Question 3 marked resolved (silent no-op, `T31.e`). `BL-0148` (heal-spend input binding) remains open, unresolved by this package. **`09-package-verification` (2026-07-19, [VR-1123](verification/VR-1123-infinite-mode-combat-player-health-and-economy.md)): `RETURNED`.** Independent live drive through the real `MODE SELECT`→...→`INTRO`→`PLAYING` UI path found the "initial Infinite Mode entry" `inf_record_combat_entry` call site (inside `st_infinite_seed_entry`'s A-confirm) fires *before* `st_intro` sets `PLAYER_X`/`PLAYER_Y` to their real spawn value `(76, 80)` — `COMBAT_ENTRY_X`/`Y` record `(0, 0)` for a session's first region, so a zero-health setback before the player ever crosses a region boundary teleports them to `(0, 0)`, not their real entry point. `T31.c` didn't catch this because it force-writes `COMBAT_ENTRY_X`/`Y` directly. The other five recording sites are correctly ordered — isolated to this one call site. Harvested as `BL-0154` (Medium). **Remediated same day (`08-code-implementation`, `BL-0154`):** moved the "initial entry" `inf_record_combat_entry` call from `st_infinite_seed_entry`'s own A-confirm handler into `st_intro`'s own A-press handler, immediately after the real `PLAYER_X`/`PLAYER_Y` write — `check_collisions`-style minimal, single-site fix, no other call site touched. New regression check **`T31.g`** drives the real `MODE SELECT`→`COMBAT MODE CONFIRM` (confirm Y)→`INFINITE SEED ENTRY`→`INTRO`→`PLAYING` path (not a direct-invoke force) and confirms `COMBAT_ENTRY_X`/`Y` exactly match the player's real spawn position `(76, 80)`. **364/364 suite passes** (363 + `T31.g`), ROM unchanged 32158/32768 (net-zero: one `CALL` removed, one added). `GDS-07` §7k updated to describe the corrected six-site ordering. Own `09-package-verification` pass owed (Pass 2). **`09-package-verification` Pass 2 (2026-07-19, [VR-1123 Pass 2](verification/VR-1123-infinite-mode-combat-player-health-and-economy-pass2.md)): `VERIFIED`.** Fresh session (`pyboy` installed, not preinstalled); 387/387 suite, ROM 32768 bytes. Direct code read confirmed the `BL-0154` fix (call site moved from `st_infinite_seed_entry` to `st_intro`, after the real `PLAYER_X`/`PLAYER_Y` write); `T31.a`-`g` reconfirmed passing. Independent live drive (own script, not just `T31.g`) reproduced Pass 1's own exact repro end-to-end — real UI path to `PLAYING`, real per-frame mob contact to zero health, real `inf_health_setback` — and confirmed the player now lands at `(76, 80)`, the real spawn point, not `(0, 0)`. No scope excursion, no findings. |
| [IP-1124](packages/IP-1124-infinite-mode-combat-save-persistence.md) | Infinite Mode Combat: Save Persistence | `08-code-implementation` | **VERIFIED** ([VR-1124](verification/VR-1124-infinite-mode-combat-save-persistence.md)) | IP-1121 (VERIFIED), IP-1122 (VERIFIED), IP-1123 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | **Implemented 2026-07-19.** New SRAM `SRAM_COMBAT_MODE`/`SRAM_MOB_COUNT`/`SRAM_MOB_DATA`/`SRAM_WEAPON_TIER`/`SRAM_PLAYER_HEALTH` (`0xA350`–`0xA371`); `SAVE_VERSION_VAL` `0x05`→`0x06` (sixth bump since ship, extending `IP-1010`→`IP-1050`→`IP-9110`→`IP-1104`'s own monotonic sequence). `PROJ_*` fields deliberately not persisted, mirroring `INF_MZ_RESULT`'s own transient-state precedent. `save_to_sram`/`try_load_save` both extended inside their existing single MBC1-enable bracket, nested inside the existing `GAME_MODE==1` gate. `SRAM_COMBAT_MODE` always written/restored as a flag (mirrors `SRAM_GAME_MODE`'s own precedent); the four data fields additionally gated on `COMBAT_MODE` itself on both save and load — restoring them unconditionally would overwrite their own correct boot-time defaults with stale SRAM bytes whenever a non-combat Infinite Mode save loads. **Two corrections found and fixed during implementation, not silently patched over** (full detail in `GDS-07` §7o): (1) the package's own §6 described the five persisted fields as one 34-byte contiguous WRAM span — direct code read confirmed `PROJ_ACTIVE`/`PROJ_X`/`PROJ_Y`/`PROJ_STEP_X` (`IP-1122`/`IP-1128`) actually sit in between `MOB_DATA` and `WEAPON_TIER`, so it's really two separate transfers (30 bytes, then 2 bytes), not one; (2) the combat-state restore had to be sequenced *after* `try_load_save`'s existing `inf_ensure_window` call, not before — that call's own center-cell recompute unconditionally invokes `inf_materialize_mobs`, which (once `COMBAT_MODE` is restored) would otherwise immediately clobber the just-restored `MOB_DATA`/`MOB_COUNT` with a freshly re-materialized set; `COMBAT_MODE` restores between `inf_ensure_window` and `inf_record_combat_entry` so the latter still correctly fires. New suite `T32` (4 checks: `T32.a` real two-instance save/load round trip incl. mob state/weapon tier/player health, `T32.b` projectile non-persistence, `T32.c` pre-combat-mode (`0x05`) save compatibility mirroring `T27.d`'s own synthetic-fixture pattern, `T32.d` `COMBAT_MODE`-off-at-save-time non-write, read directly from the raw `.ram` file). **393/393 suite passes.** ROM builds at 32768 bytes (32414 used, unchanged from `IP-1129`'s own footprint — the new code fit within existing padding/alignment slack). `FR-11600` → Implemented; RTM row filled (`IP-1124`/`T32.a-d`); `FR-11510`'s own RTM/status note updated (its save/load half is now closed, not merely pending); `FS-112` metadata updated (Workflow E implemented-by pointer, `SAVE_VERSION_VAL` bump confirmed); `GDS-07` new §7o added. **This closes the original six-package combat sub-mode critical path in full** — `IP-1120`–`IP-1123`/`IP-1125`/`IP-1126`/`IP-1128`/`IP-1129` all `VERIFIED`, `IP-1124` now `COMPLETE`, own `09-package-verification` pass owed. **Independently verified 2026-07-20** ([VR-1124](verification/VR-1124-infinite-mode-combat-save-persistence.md), fresh session, 404/404 suite, ROM rebuild 32768 bytes) — every DoD/checklist item confirmed by direct code read; single MBC1 bracket preserved in both routines; `PROJ_*` fields confirmed never referenced; implementing commit's own diff confirmed to touch exactly one pre-existing line (the version bump), everything else additive. `VERIFIED`, no findings. This closes every package in the Infinite Mode Combat Sub-Mode delta except `IP-1127`'s own still-owed `09` pass. |
| [IP-1120](packages/IP-1120-infinite-mode-combat-mode-gating.md) | Infinite Mode Combat: Mode Gating & UI | `08-code-implementation` | **VERIFIED** ([VR-1120](verification/VR-1120-infinite-mode-combat-mode-gating.md)) | IP-1121 (VERIFIED), IP-1100 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | **2026-07-18 Blocking Report** (`BL-0153`): the original design (a new `combat_mode_confirm_screen()` `ALL_SCREENS` entry) overflowed the ROM by 542 bytes. **Re-planned same day**: reuses `mode_select_screen`'s own array + a `memcpy` runtime text overlay, no new `ALL_SCREENS` entry. **Implemented 2026-07-18** (same session as the re-plan): `GS_COMBAT_MODE_CONFIRM=12`, `CMC_CURSOR` (`0xC6DD`) defined; `ms_infinite` retargeted (+ `CMC_CURSOR` reset); `st_combat_mode_confirm` (UP/DOWN/A/B, mirrors `st_mode_select` exactly); three inline literal-text blobs (`cmc_str_title`/`cmc_str_no`/`cmc_str_yes`, `char_to_tile()`-encoded, "NO"/"YES" pre-padded with `TL_BG_BLANK` so a single `memcpy` both writes the new label and erases the longer original's trailing characters) behind a `JR`-skip; `cmc_on_entry` (three `memcpy` calls, unconditional every `dsr_cmc` dispatch since `copy_screen` re-blits the reused array's own baked-in text every redraw) + `draw_combat_confirm_cursor` (mirrors `draw_mode_select_cursor` exactly); `build_rom.py` patches `'cmc_t'`/`'cmc_a'` to `screen_addrs['mode_select']` — confirmed by direct build output: no `combat_mode_confirm` line printed, `ALL_SCREENS` length unchanged. **Found and fixed a genuine planning gap during implementation**: the package's own Verification Checklist claimed "full suite passes, including `T25` unmodified" — but `ms_infinite`'s own retarget (§5, explicitly specified) necessarily changes `T25.b2a`'s own expected `GAMESTATE` (11→12) and breaks the implicit "confirm infinite → land in INFINITE SEED ENTRY" assumption three other `T25` sites and the shared `enter_infinite_mode` helper (6 downstream callers across `T26`/`T27`) all made. Fixed in scope (a necessary consequence of this package's own specified behavior, not an unrelated bug): `T25.b2a` corrected to `GS==12` with a comment explaining why; `T25.d1`/`T25.e1`/`T25.f` and `enter_infinite_mode` each gained one extra `A`-press (confirming the new state's default "N") before continuing their own existing navigation. **363/363 suite passes** (new suite `T33`, 14 checks: `T33.a`-`h` per the package's own Tests to Add, several with sub-assertions). ROM 32158/32768 (610 headroom, +256 bytes — matches the re-plan's own estimate almost exactly). `FR-11100` → Implemented; RTM row updated; `FS-112` metadata updated (Open Question 1's own implementation now recorded); `GDS-07` §7l added. **Independently verified 2026-07-19** ([VR-1120](verification/VR-1120-infinite-mode-combat-mode-gating.md), fresh session, 363/363 suite, own live PyBoy drive/screenshots) — `VERIFIED`, no findings. **This closes all six combat sub-mode packages to at least `COMPLETE`; IP-1122/IP-1123 still owe their own `09` passes before `IP-1124` becomes eligible.** |
| [IP-1126](packages/IP-1126-infinite-mode-combat-mob-movement.md) | Infinite Mode Combat: Mob Movement | `08-code-implementation` | **VERIFIED** ([VR-1126](verification/VR-1126-infinite-mode-combat-mob-movement.md)) | IP-1121 (VERIFIED) | **AUTHORIZED (G3, user go-ahead, 2026-07-19 — "Authorized but don't iterate now"; build proceeded on the user's later "Run the pipeline manager")** | **Implemented 2026-07-19**: `MOB_MOVE_TIMER` (`0xC6DE`, boot-cleared) defined; `MOB_MOVE_INTERVAL`/`MOB_MOVE_STEP` (8 frames / 1 pixel, plain Python constants) defined; `inf_mob_move` hooked into `st_playing`'s per-frame chain alongside `inf_projectile_update`/`inf_mob_contact_check` (gated on `COMBAT_MODE`). Per active `MOB_DATA` slot, moves one `MOB_MOVE_STEP`-pixel step on the dominant axis (`|dx|` vs `|dy|`, ties favor X) toward the player every `MOB_MOVE_INTERVAL` frames; holds still when exactly coincident (`dx==0, dy==0`, resolving `FS-112` Open Question 4). **Timer semantics refined during implementation** (not a drift from the package's own design, a correction found via `T35.a`'s own written expectation): a naive "decrement, then separately check-then-reset" design would have made the effective cadence `MOB_MOVE_INTERVAL+1` frames and would not move on the very frame a forced `MOB_MOVE_TIMER=1` reaches 0 — replaced with a "timer already 0, or decrementing reaches 0, both move immediately" rule that gives an exact `MOB_MOVE_INTERVAL`-frame gap between moves and satisfies `T35.a`/`T35.f` exactly as specified. Register-allocation note: `CP_H`/`CP_L`/`SUB_H` have no named `gbc_lib.py` wrapper — used via `rom.emit(0xBC)`/`0xBD`/`0x94`, the same escape hatch `inf_mob_contact_check` already established for `SUB_H`. The loop-back branch (`imv_loop`) exceeds `JR`'s ±127-byte range given the routine's own size — used `JP_C` instead, a first for this codebase's own combat-mode routines. **373/373 suite passes** (new suite `T35`, 9 checks: `T35.a`-`d` per-axis movement/dominance, `T35.e` coincident hold-still, `T35.f` sub-interval no-move + exact decrement, `T35.g` `COMBAT_MODE`-off no-op, `T35.h` inactive-slot exclusion, `T35.i` independent live `PyBoy` drive through the real per-frame chain — found and fixed a test-only bug during authoring, a stray menu-navigation `A`-press latching into a real projectile fire that clipped the forced test mob before the movement window completed; fixed by explicitly clearing `PROJ_ACTIVE` in the test's own setup, no product-code change). ROM 32414/32768 (headroom 354 bytes, down from 610). `FR-11210` → Implemented (concrete `MOB_MOVE_INTERVAL=8`/`MOB_MOVE_STEP=1` recorded); RTM row updated; `FS-112` Open Question 4 marked Resolved (both its jitter-case edge-case paragraph and its §19 entry); `GDS-07` new §7m added. **Outstanding**: `NFR-1500`'s own direct per-frame cycle-count measurement remains `UNCONFIRMED` — a pre-existing gap spanning the whole combat sub-mode (`IP-1121`/`IP-1122`/`IP-1123` too), not created by this package and outside its own named scope, but this package does add further per-frame cost on top of it. **`09-package-verification` (2026-07-19, [VR-1126](verification/VR-1126-infinite-mode-combat-mob-movement.md)): `VERIFIED`.** Fresh session; 387/387 suite (`T35.a`-`i` reconfirmed); ROM 32768 bytes. Direct code read confirmed the dominant-axis/tie-break logic, the coincident hold-still case, and the `COMBAT_MODE`/inactive-slot gating all match §6/§10 exactly; implementing commit's `asm_game.py` diff confirmed purely additive (no line removed/modified) — `inf_mob_render`/`inf_projectile_hittest`/`inf_mob_contact_check` and `MOB_DATA`'s own stride all confirmed untouched. No scope excursion, no findings. |
| [IP-1127](packages/IP-1127-infinite-mode-combat-post-contact-protection.md) | Infinite Mode Combat: Post-Contact Player Protection | `08-code-implementation` | **VERIFIED** ([VR-1127](verification/VR-1127-infinite-mode-combat-post-contact-protection.md)) | IP-1121 (VERIFIED), IP-1123 (VERIFIED) | **AUTHORIZED (G3, user go-ahead, 2026-07-19, same session — asked after `IP-1123` reached `VERIFIED`)** | **Implemented 2026-07-19.** Extends `inf_mob_contact_check` (`IP-1123`) with a per-mob cooldown bit (new parallel table `MOB_CONTACT_FLAGS`), an invincibility countdown (`PLAYER_INVINCIBLE`), and a knockback push (clamped to `handle_play_input`'s own existing position bounds). **WRAM addresses re-derived at build time** (`BL-0163`): `IP-1128` shipped first and claimed the package's own originally-planned `0xC6DF`–`0xC6E1` for real, so `PLAYER_INVINCIBLE`/`MOB_CONTACT_FLAGS` moved to the next free bytes, `0xC6E2`–`0xC6E3`. `check_collisions`/`MOB_DATA`'s own layout confirmed byte-for-byte unchanged by direct diff. **Found-and-fixed interaction, named explicitly:** a lethal hit (triggering `inf_health_setback`) skips knockback on that path only — applying it against the pre-setback position would have silently displaced the player off the setback's own just-restored entry point. **Two test-only bugs found and fixed during authoring:** several `T36` checks first read state after knockback had already (correctly) separated the player from the mob, breaking a same-position follow-up mob placement's own overlap assumption — fixed by re-pinning the player's own test position between invocations where genuinely continued overlap was the point being tested. New suite `T36` (12 checks incl. `T36.j`, a live PyBoy drive through the real production per-frame chain). **404/404 suite passes.** ROM builds at exactly 32768 bytes (32670 used, **98 bytes headroom — the tightest margin of any package in this tranche**). `FR-11410` → Implemented; RTM row filled; `FS-112`/`GDS-07` §7p updated (also closes `BL-0163`, a stale doc mislabel found during `IP-1124`). `BL-0158` remained open pending this package's own `09-package-verification` confirming the fix live, per this package's own §9. **This closed every package in the Infinite Mode Combat Sub-Mode delta (`IP-1120`–`IP-1129`) to at least `COMPLETE`.** **Independently verified 2026-07-20** ([VR-1127](verification/VR-1127-infinite-mode-combat-post-contact-protection.md), fresh session, 404/404 suite, ROM rebuild 32768 bytes) — every DoD/checklist item confirmed by direct code read; `check_collisions`/`MOB_DATA` layout confirmed unmodified; knockback clamp bounds confirmed matching `handle_play_input`'s own; `MOB_CONTACT_FLAGS` confirmed a genuinely separate table. `VERIFIED`, no findings; closes `BL-0158`. **This closes every package in the Infinite Mode Combat Sub-Mode delta (`IP-1120`–`IP-1129`) to `VERIFIED` — the full ten-package tranche is done; `10-integration-review` is the natural next step.** |
| [IP-1128](packages/IP-1128-infinite-mode-combat-weapon-directionality.md) | Infinite Mode Combat: Weapon Directionality | `08-code-implementation` | **VERIFIED** ([VR-1128](verification/VR-1128-infinite-mode-combat-weapon-directionality.md)) | IP-1122 (VERIFIED) | AUTHORIZED (G3, user go-ahead, 2026-07-19 — "Authorize both") | Implemented 2026-07-19 (`08-code-implementation`). Widens weapon fire to 8 compass directions derived from player movement. New `PLAYER_FACING_X`/`PLAYER_FACING_Y`/`PROJ_STEP_Y` (`0xC6DF`–`0xC6E1`, **now claimed for real** — resolves the prior planning-time collision with `IP-1127`, which must re-derive its own range at build time); `PROJ_DIR` (`0xC6D8`) repurposed in place as `PROJ_STEP_X`, redefined from 2-value left/right to a raw signed-step encoding — supersession sweep confirmed exactly two consumers, both rewritten, `inf_projectile_hittest` confirmed byte-for-byte unchanged. Diagonal projectile motion via simultaneous independent per-axis stepping (this codebase's own existing 2D-movement idiom). New suite `T37` (9 checks incl. a live PyBoy drive T37.i); `T30.a`/`T30.b`/`T30.c`/`T30.c2`/`T30.d` corrected for the new encoding. ROM builds at 32768 bytes; full suite 382/382 passing. **`09-package-verification` (2026-07-19, [VR-1128](verification/VR-1128-infinite-mode-combat-weapon-directionality.md)): `VERIFIED`.** Fresh session; 387/387 suite; `T37.a`-`i` reconfirmed, `T30.a`/`b`/`c`/`c2`/`d` corrections confirmed legitimate (re-encoded, not weakened). `inf_projectile_hittest` and `PLAYER_DIR`'s own write sites confirmed byte-for-byte untouched by direct diff. No scope excursion, no findings. |
| [IP-1129](packages/IP-1129-infinite-mode-combat-weapon-tier-funding.md) | Infinite Mode Combat: Weapon-Tier Funding Economy | `08-code-implementation` | **VERIFIED** ([VR-1129](verification/VR-1129-infinite-mode-combat-weapon-tier-funding.md)) | IP-1122 (VERIFIED), IP-1103 (VERIFIED) | AUTHORIZED (G3, user go-ahead, 2026-07-19 — "Authorize both") | Implemented 2026-07-19 (`08-code-implementation`). New `inf_tier_spend`, mirroring `inf_heal_spend`'s own exact structure — decrements `RUNNING_TREASURE_COUNT`, increases `WEAPON_TIER` capped at 3, **spending even at the cap** (matches `FR-11500`'s own real shipped precedent, `T31.d2` — not a no-op). No new WRAM. New suite `T38` (5 checks); `T38.e` scoped to the in-session persistence half only (`FR-11600` save/load still unimplemented, named explicitly). Not yet player-reachable (`BL-0148`-class input-binding gap, named not resolved). ROM builds at 32768 bytes; full suite 387/387 passing. **`09-package-verification` (2026-07-19, [VR-1129](verification/VR-1129-infinite-mode-combat-weapon-tier-funding.md)): `VERIFIED`.** Fresh session; 387/387 suite; `T38.a`-`e` reconfirmed. Implementing commit's own `asm_game.py` diff confirmed purely additive (0 removed/modified lines) — `inf_tier_spend` confirmed byte-for-byte structural mirror of `inf_heal_spend`; `inf_projectile_hittest`/`WEAPON_TIER`'s boot-init confirmed untouched. No scope excursion, no findings. This closes the session's `09-package-verification` backlog to zero. |
| [IP-8010](packages/IP-8010-point-in-box-hittest-deduplication.md) | Point-in-Box Hit-Test Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8010](verification/VR-8010-point-in-box-hittest-deduplication.md)) | none | **AUTHORIZED (G3, user "Authorized all refactoring," 2026-07-20)** | **Implemented 2026-07-20.** New shared subroutine `pib_reg_minus_origin` extracted; `check_collisions`/`inf_mob_contact_check` both rewritten to call it (`LD_HL_nn(PLAYER_X); CALL('pib_reg_minus_origin'); J{R,P}_NZ(skip)`), preserving each site's own branch instruction (`JR` vs `JP`) and register-preservation requirements (`B`/`C` survive at both sites, confirmed by direct read before editing). `inf_projectile_hittest` confirmed zero diff lines — its own reverse-order test is a different, non-commutable predicate, correctly left out of scope (named explicitly in the package's own §13, not silently dropped). **Equivalence evidence:** `test_rom.py` itself has zero diff lines; full suite **404/404**, identical check-name set to the pre-refactor baseline (`diff` clean); ROM builds at exactly 32768 bytes, **same 32670 bytes used as the pre-refactor baseline** (net-zero at the section-total level — the ~9-10-byte-per-site savings and the new ~16-byte shared body absorbed within existing alignment slack, not a target that was "hit," an observed outcome); the full byte-level diff (6365 differing bytes, offsets 0x14e–0x21d7) is pure address-relocation cascade from the code-layout shift (every downstream label's resolved address moves), not a logic change — confirmed by the identical passing suite. `check_collisions`'s/`inf_mob_contact_check`'s diff hunks confirmed to touch only the three intended regions (the new subroutine + the two call-site rewrites). No documentation update needed — `GDS-07`'s/`ADS-002`'s own references to "the asymmetric point-in-box technique" describe the technique conceptually, still accurate; neither cites the removed opcode-level implementation. **`09-package-verification` (2026-07-20, [VR-8010](verification/VR-8010-point-in-box-hittest-deduplication.md)): `VERIFIED`.** Same-session (degraded independence explicitly accepted, per the user's own continuous-iteration instruction); ROM rebuild byte-identical (SHA-256 match), 404/404 suite, `pib_reg_minus_origin`/both call sites/`inf_projectile_hittest`'s own non-involvement all directly reconfirmed. No findings. |
| [IP-8020](packages/IP-8020-abs-delta-from-player-deduplication.md) | Absolute-Delta-From-Player Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8020](verification/VR-8020-abs-delta-from-player-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization for this session's further well-scoped refactor work)** | **Implemented 2026-07-20.** New shared subroutine `abs_delta_from_player` extracted; `inf_mob_move`'s own `imv_ax_*`/`imv_ay_*` block and the knockback block's own `ikb_ax_*`/`ikb_ay_*` block (in `inf_mob_contact_check`) both replaced by a single `CALL('abs_delta_from_player')`. Diff confirmed to touch only the new subroutine plus the two call-site rewrites (direct diff of removed labels shows exactly the twelve `imv_a[xy]_*`/`ikb_a[xy]_*` labels, nothing else). **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404**, identical check-name set to the pre-refactor baseline; ROM builds at exactly 32768 bytes, same 32670 bytes used as baseline (net-zero at the section-total level again, same alignment-slack absorption as `IP-8010`). No documentation update needed (behavior/technique unchanged, only implementation). **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted per the user's own continuous-iteration instruction): `VERIFIED`.** ROM rebuild reproduced deterministically; 404/404 reconfirmed; both call sites and the subroutine's own register contract (C=ax/L=ay, preserves B/D/E) directly re-derived from source. No findings. |
| [IP-8030](packages/IP-8030-treasure-spend-gate-deduplication.md) | Treasure-Spend Gate-and-Decrement Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8030](verification/VR-8030-treasure-spend-gate-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | **Implemented 2026-07-20.** New shared subroutine `treasure_spend_gate_and_decrement` (with the required explicit `OR_A()` re-assertion before its success-path `RET()`, per the package's own §5 correctness note); `inf_heal_spend`/`inf_tier_spend` both replaced their own gate/decrement prefix with `CALL(...); RET_Z()`, keeping their own capped-increment tails unchanged. **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404**, identical check-name set to baseline; ROM 32768 bytes, 32670 used (unchanged). `T31.d2`/`T38.b` ("spends even at the cap") both reconfirmed passing. **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings. |
| [IP-8040](packages/IP-8040-idx5-addressing-deduplication.md) | Index*5 Addressing Arithmetic Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8040](verification/VR-8040-idx5-addressing-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | **Implemented 2026-07-20.** New shared subroutine `idx5_to_hl`; `czt_region_hl`/`setup_zone_collects`/`gw_neighbor_hl`/`ilmc_evict` all rewritten to call it. **Scope corrected during execution**: `dsr_p` reverted after `T24.c1` (a static characterization test pinning its exact instruction sequence) failed — filed as `BL-0174`, not a defect, `dsr_p` confirmed zero diff lines. **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404** incl. `T24.c1` itself now passing; ROM 32768 bytes, 32670 used (unchanged from baseline). **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings beyond `BL-0174`. |
| [IP-8050](packages/IP-8050-keyitem-flags-addressing-deduplication.md) | KEYITEM_FLAGS Addressing Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8050](verification/VR-8050-keyitem-flags-addressing-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | **Implemented 2026-07-20.** New shared subroutine `keyitem_flags_hl` extracted; all six sites (`check_collisions`'s own KeyItem-flag-set branch, `setup_zone_collects`'s own KeyItem-collected check, the three `ki_passA_*` maze-generation branches, and the fallback-fill site near `ki_passB_done`) rewritten to call it immediately after each site's own existing index-load into `A`. **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404**, identical check-name set to baseline (confirmed via `diff` against the saved pre-refactor check-list); ROM builds at exactly 32768 bytes, same 32670 bytes used as baseline (net-zero at the section-total level, same alignment-slack absorption as the prior `IP-8xx0` packages). `git diff asm_game.py` confirmed to touch only the new subroutine plus the six call-site substitutions, nothing else. No documentation update needed. **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted per the user's own continuous-iteration instruction): `VERIFIED`.** No findings. |
| [IP-8060](packages/IP-8060-seed-cursor-arrow-deduplication.md) | Seed-Cursor-Arrow Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8060](verification/VR-8060-seed-cursor-arrow-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | **Implemented 2026-07-20.** New shared subroutine `dsd_seed_cursor_arrow` extracted; `draw_sse_digits`/`draw_ise_digits` both rewritten to call it. **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404**, identical check-name set to baseline; ROM builds at exactly 32768 bytes, 32670 used (unchanged, alignment-slack absorption). `git diff` confirmed exactly the new subroutine plus two call-site substitutions. **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings. |
| [IP-8070](packages/IP-8070-gw-neighbor-read-deduplication.md) | `gw_neighbor_hl` Read-Wrapper Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8070](verification/VR-8070-gw-neighbor-read-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | **Implemented 2026-07-20.** New shared subroutine `gw_read_neighbor`; `maze_try_loop`/`maze_prune_dir` rewritten to call it. **Scope corrected during execution**: `ki_passA_dir` reverted after `T12.e` failed — `gw_read_neighbor`'s use of `B` as scratch clobbers that site's own live `has_child` loop accumulator (a register-liveness conflict invisible from the block's own immediate context); confirmed zero diff lines at that site. **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404** incl. `T12.e`, identical check-name set to baseline; ROM 32768 bytes, 32670 used (unchanged). **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings beyond the scope correction itself. |
| [IP-8080](packages/IP-8080-ledger-entry-write-deduplication.md) | Ledger-Entry Write-Block Deduplication (refactor) | `08-refactoring` | **VERIFIED** ([VR-8080](verification/VR-8080-ledger-entry-write-deduplication.md)) | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | **Implemented 2026-07-20.** New shared subroutine `write_ledger_entry_at_hl` extracted; `ilmc_notfound`/`ilmc_evict` both rewritten to call it. **Equivalence evidence:** `test_rom.py` zero diff; full suite **404/404**, identical check-name set to baseline; ROM builds at exactly 32768 bytes, 32670 used (unchanged). `git diff` confirmed exactly two hunks (the two call-site rewrites + the new subroutine). **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings. |
| [IP-8090](packages/IP-8090-pipeline-journal-archive-split.md) | Pipeline Journal Archive Split (refactor, doc-scope) | `08-refactoring` | **VERIFIED** ([VR-8090](verification/VR-8090-pipeline-journal-archive-split.md)) | none | **AUTHORIZED (user directly approved the "archive-split" approach via `AskUserQuestion`, 2026-07-20)** | **Implemented 2026-07-20.** Split `docs/pipeline/pipeline-journal.md` (541KB) into a live file (108.7KB — Position section unchanged + runs #240-284 + a pointer note) and a new `pipeline-journal-archive.md` (433KB — runs #0-239, verbatim). **Equivalence evidence:** full byte-for-byte `diff` of both halves against the pre-refactor file's own corresponding line ranges is empty in both directions; Position section confirmed byte-identical; the only bytes added anywhere are the new pointer/header text (+821 bytes total), zero original content lost. `00-pipeline-manager/SKILL.md` and `docs/INDEX.md` updated to document/link the new archive. **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings. |
| [IP-8100](packages/IP-8100-backlog-archive-split.md) | Backlog Archive Split (refactor, doc-scope) | `08-refactoring` | **VERIFIED** ([VR-8100](verification/VR-8100-backlog-archive-split.md)) | none | **AUTHORIZED (user directly approved the "archive-split" approach via `AskUserQuestion`, 2026-07-20)** | **Implemented 2026-07-20.** Split `docs/pipeline/backlog.md` (377KB, 182 entries) into a live file (117KB — 65 open-status rows: 48 `SCHEDULED`, 15 `DEFERRED`, 2 `IN PIPELINE`) and a new `backlog-archive.md` (117 `DONE` rows, 0 `REJECTED`). **Equivalence evidence:** programmatic partition by the true trailing Status-field text (robust to two pre-existing malformed rows, `BL-0127`/`BL-0128`, whose Status field had absorbed leaked Disposition prose — resolved correctly via suffix match, not a naive column split); full `diff` of both halves against the pre-refactor file's own corresponding rows is empty in both directions; 117+65=182 accounts for every row exactly once. Header text updated to describe the new archive convention (was "rows are never deleted," now "rows are never deleted, only archived once resolved"). `00-pipeline-manager/SKILL.md` and `docs/INDEX.md` updated. **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings. |
| [IP-8110](packages/IP-8110-roadmap-cell-compaction.md) | ROADMAP.md Cell Compaction (refactor, doc-scope) | `08-refactoring` | **VERIFIED** ([VR-8110](verification/VR-8110-roadmap-cell-compaction.md)) | none | **AUTHORIZED (user directly approved the "compact + pointer" approach via `AskUserQuestion`, 2026-07-20)** | **Implemented 2026-07-20.** Compacted ROADMAP.md's seven worst-offender Status cells (`IM-00`, `IP-xxxx`, `RV-INTEG`, `R201-R220`, `R101-R115`, `ADR-xxxx`, `IM-01`) to a short current-state summary + pointer to the owning doc/theme `INDEX.md` that already holds the full history. **Equivalence evidence:** every fact independently re-derived from the owning doc/INDEX before trimming (package counts parsed from `00-master-build-plan.md`, ADR count from `ls docs/architecture/adr/`, integration-review count from `ls docs/reviews/`); file shrunk 59,787 → 24,554 bytes (59% reduction); `git diff` confirms exactly the 7 target lines changed, nothing else. Filed one incidental out-of-scope finding (`BL-0183`, the separately-stale `VR-xxxx` row) rather than silently fixing it beyond this package's own named scope. **`09-package-verification` (2026-07-20, same-session, degraded independence explicitly accepted): `VERIFIED`.** No findings within scope. |
...
**Authorization state: the original six packages (`IP-1120`–`1125`) are AUTHORIZED** (G3, user
"Yes, build all six," 2026-07-17). Build order per the critical path above: `IP-1125` first
(dependency root), then `IP-1121`, then `IP-1122`/`IP-1123` (parallel-eligible), then `IP-1124`
last; `IP-1120` in parallel once `IP-1121` lands.

**`IP-1126`/`IP-1127` (planned 2026-07-19) are a separate, later delta — NOT covered by the
"build all six" authorization.** `IP-1126` branched off `IP-1121` independently of the original
critical path (parallel-eligible with everything above); received its own G3 go-ahead 2026-07-19
and is now **`VERIFIED`** (independently verified 2026-07-19, `VR-1126`). `IP-1127` branched off
`IP-1121` and `IP-1123` — structurally unblocked 2026-07-19 (`IP-1123` reached `VERIFIED`, Pass 2),
received its own G3 go-ahead the same session, and is now `COMPLETE` (own `09-package-verification`
pass owed). Neither extends the original critical path (`IP-1124` does not depend on either).

**`IP-1128`/`IP-1129` (planned 2026-07-19, later the same session) are a third, independent
delta — also NOT covered by the "build all six" authorization.** Both branch off `IP-1122`
(`IP-1129` additionally off `IP-1103`), both already `VERIFIED` — `IP-1128`/`IP-1129` are
therefore both immediately `READY`, unlike `IP-1127`. Neither extends the original critical path.
...
| Package | Title | Owner (08 peer) | Status | Depends on | Authorized? | Notes |
|---|---|---|---|---|---|---|
| [IP-9210](packages/IP-9210-automatic-weapon-tier-upgrade.md) | Automatic Weapon-Tier Upgrade Trigger (`BL-0148`/`ADR-0022`) | `08-code-implementation` | **IN PROGRESS** | `IP-1129` (`VERIFIED`) | **AUTHORIZED (G3, user specified the exact 1/3 threshold curve directly, 2026-07-20 -- treated as authorization to build with that curve)** | Rewrites `inf_tier_spend` in place: automatic per-frame check during `COMBAT_MODE`, threshold-crossing (1 treasure for tier 1→2, 3 for tier 2→3), true no-op once `WEAPON_TIER == 3` (unlike `inf_heal_spend`'s own spend-even-at-cap shape). New unconditional `CALL('inf_tier_spend')` from `st_playing`. `T38` suite rewritten (`T38.a`-`f`) to test the automatic/threshold behavior in place of the retired manual-spend shape. No new WRAM. |
