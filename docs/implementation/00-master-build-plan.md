...
| [IP-8060](packages/IP-8060-seed-cursor-arrow-deduplication.md) | Seed-Cursor-Arrow Deduplication (refactor) | `08-refactoring` | **IN PROGRESS** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | Filed from `BL-0177` (run #283, full-codebase survey). Extracts `draw_sse_digits`/`draw_ise_digits`'s own identical 5-instruction cursor-arrow tail into `dsd_seed_cursor_arrow`. Expected small net ROM decrease. |
| [IP-8070](packages/IP-8070-gw-neighbor-read-deduplication.md) | `gw_neighbor_hl` Read-Wrapper Deduplication (refactor) | `08-refactoring` | **IN PROGRESS** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | Filed from `BL-0178` (run #283, full-codebase survey). Extracts the "read neighbor region in `GW_MAZE_DIR` direction" idiom into `gw_read_neighbor`. Expected small net ROM change either direction. |
