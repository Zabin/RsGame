...
| [IP-1126](packages/IP-1126-infinite-mode-combat-mob-movement.md) | Infinite Mode Combat: Mob Movement | `08-code-implementation` | **READY** | IP-1121 (VERIFIED) | **NOT AUTHORIZED** — new scope beyond the original "Yes, build all six" go-ahead (2026-07-17, `IP-1120`–`1125` only); `FR-11210` baselined 2026-07-19 in direct response to a user-filed gap (`BL-0156`). | Planned 2026-07-19 (`07-implementation-planning`). Per-frame mob movement toward the player: dominant-axis single-step, adjustable `MOB_MOVE_INTERVAL`/`MOB_MOVE_STEP` (starting values 8 frames / 1 pixel), holds still once coincident with the player (resolves `FS-112` Open Question 4). New WRAM `MOB_MOVE_TIMER` (`0xC6DE`). New suite `T35` (9 checks incl. a live PyBoy drive). Sole dependency `IP-1121` already `VERIFIED` — `READY` pending G3. |
...
"build all six" authorization.** `IP-1126` branches off `IP-1121` independently of the original
critical path (parallel-eligible with everything above, `READY` pending its own G3 ask).
`IP-1127` branches off `IP-1121` and `IP-1123` (the latter still verification-owed) — `BLOCKED`
until `IP-1123` reaches `VERIFIED`, independent of `IP-1126`'s own readiness. Neither extends the
original critical path (`IP-1124` does not depend on either).
