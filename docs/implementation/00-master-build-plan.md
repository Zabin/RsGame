...
| [IP-1082](packages/IP-1082-maze-blocked-edge-indicator-render.md) | Maze-blocked edge indicator — render (FS-108 / FEAT-2100 / BL-0075) | `08-code-implementation` | **READY** (IP-1081 now VERIFIED) | IP-1081 (VERIFIED), IP-1080 (VERIFIED) | **YES — explicit user G3, 2026-07-12 (BL-0092, "Yes, build both")** | Planned 2026-07-12. `draw_region_arrows`'s blocked-case render branch, reusing `IP-1080`'s own `DRA_ROW`/`DRA_COL` arithmetic and the existing `ARROW_ADDR_*`/`_arrow_write` machinery — no new WRAM, no new patch-point kind. Corrects `T20.b`'s own now-stale "no arrow" assertion, adds `T20.e`. Closes `BL-0075` and FR-2330 in full once `VERIFIED`. |
