...
| [IP-1120](packages/IP-1120-infinite-mode-combat-mode-gating.md) | Infinite Mode Combat: Mode Gating & UI | `08-code-implementation` | **READY** (re-planned 2026-07-18, `BL-0153`) | IP-1121 (VERIFIED), IP-1100 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17 — re-plan is a strategy change within existing scope, not new scope, mirroring `IP-1022`'s own `ADR-0020` re-plan precedent)** | **2026-07-18 Blocking Report** (see prior journal/backlog entry `BL-0153`): the original design (a new `combat_mode_confirm_screen()` `ALL_SCREENS` entry) overflowed the ROM by 542 bytes against 866 bytes of headroom. **Re-planned same day (`07-implementation-planning`):** `COMBAT MODE CONFIRM` now reuses `mode_select_screen`'s own already-registered tile/attr array as its base (via a second `_dsr_screen` call site whose `'cmc_t'`/`'cmc_a'` patches resolve to the *same* address pair `'ms_t'`/`'ms_a'` already use) and draws its own differing text (title + NO/YES labels) at runtime via three small inline literal-text blobs + `memcpy` (reused verbatim, unmodified) — the same "static base + runtime overlay" technique `draw_sse_digits`/`draw_ise_digits` already use for their own dynamic digit content, applied here to fixed strings. `tilemaps.py`/`ALL_SCREENS` are no longer touched at all — estimated total growth ~260-320 bytes (re-affirm exact figure at build time), comfortably inside the 866-byte headroom. New tests `T33.g`/`T33.h` added (reused-array non-corruption; overlay content correctness). All 14 fields re-derived against the current tree; package doc, TWBS, this row updated. `READY` — eligible for the next `08-code-implementation` pick. |
