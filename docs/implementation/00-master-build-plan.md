...
| [IP-9180](packages/IP-9180-infinite-mode-hud-treasure-count-digit.md) | Infinite Mode HUD target-digit convention (`BL-0144`) | `08-code-implementation` | **NOT STARTED** | IP-9170 (COMPLETE, own `09` pass owed) | **Not authorized** | Extends `IP-9170`'s own `GAME_MODE` branch in `update_status_disp`: the Infinite Mode half writes `RUNNING_TREASURE_COUNT mod 10` to the same `0x9804` cell (user decision, 2026-07-17). Single-digit-cell constraint named as an accepted scoping choice, not a defect. Awaits explicit G3; hard-sequenced after `IP-9170` (extends its exact branch structure). |
