...
| [IP-8080](packages/IP-8080-ledger-entry-write-deduplication.md) | Ledger-Entry Write-Block Deduplication (refactor) | `08-refactoring` | **IN PROGRESS** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | Filed from `BL-0179` (run #283, full-codebase survey). Extracts `inf_ledger_mark_collected`'s own duplicated 5-byte ledger-entry write block into `write_ledger_entry_at_hl`. Expected small net ROM decrease. |
