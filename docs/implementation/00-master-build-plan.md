...
| [IP-1033](packages/IP-1033-nine-biome-family-collectible-spawn-content.md) | Collectible-spawn content for the four newly-folded biome identities | `08-content-authoring` | **COMPLETE** (reworked 2026-07-17 per [VR-1033](verification/VR-1033-nine-biome-family-collectible-spawn-content.md)) | FR-4320 (baselined), IP-9070 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Implemented 2026-07-16: four `ZONE_COLLECTS`-format lists (`VILLAGE_COLLECTS`/`CAVE_COLLECTS`/`DESERT_COLLECTS`/`PLAINS_COLLECTS`, `tilemaps.py`), staged as inert data — final wiring is `IP-1022`'s own job. **`09-package-verification` (VR-1033, 2026-07-17) found the package's own "no overlap" placement claim false**: `VILLAGE_COLLECTS`'s `KeyItem` sat on a fence tile `(17,12)`, `DESERT_COLLECTS`'s star sat on a cactus tile `(4,12)`, `PLAINS_COLLECTS`'s first two entries both sat on hand-placed flowers `(3,4)`/`(3,12)`. **Reworked 2026-07-17** (`BL-0131`): repositioned the four entries — Village's `KeyItem` to `(48,64)`, Desert's entry to `(64,96)`, Plains' first two entries to `(40,40)`/`(48,112)` — each independently re-confirmed clear of every landmark by direct rendered-tile-value inspection (the actual tile ID at each collectible's exact grid position cross-checked against the screen generator's own output, not just arithmetic against the avoid-list). 309/309 suite unchanged, ROM byte-identical (31362/32768, current tree state). Own `09-package-verification` re-pass owed. |
| [IP-1022](packages/IP-1022-finite-mode-nine-identity-generation-and-dispatch.md) | Finite-mode nine-identity generation & screen dispatch | `08-code-implementation` | **BLOCKED** (`IP-1033` `COMPLETE`, own `09` pass still owed) | FR-4310/FR-4320 (baselined), IP-1033 (COMPLETE, reworked) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `generate_world`'s clamp `[0,4]`→`[0,8]`, extends `dsr_p_dispatch`'s cascade to all nine identities, splices `IP-1033`'s staged content into `ZONE_COLLECTS`'s real array. `CR-08`'s resolution unblocked this package's own planning; `IP-1033`'s reworked content (2026-07-17) is technically startable but its own `09` re-pass is still owed before splicing coordinates that already failed one verification pass. |
| [IP-1106](packages/IP-1106-infinite-mode-nine-identity-value-widening.md) | Infinite Mode nine-identity value-range widening | `08-code-implementation` | **BLOCKED** (one real prerequisite, `IP-1022`) | IP-1105 (VERIFIED), IP-1022 (BLOCKED), IP-1033 (COMPLETE, reworked) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `materialize_region`'s draw `%5`→`%9`, extends `inf_treasure_pos` to nine entries. The deepest point in this delta's own dependency chain (`IP-1033`/`IP-1105` → `IP-1022` → `IP-1106`) — shipping ahead of its own prerequisites would reintroduce the exact live-regression risk `IP-1105`'s own TWBS entry first identified. `IP-1105` `VERIFIED`; `IP-1033` reworked and `COMPLETE` (own `09` re-pass owed); `IP-1022` remains the real blocker. |
