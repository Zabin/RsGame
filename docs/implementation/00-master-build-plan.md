...
| [IP-8040](packages/IP-8040-idx5-addressing-deduplication.md) | Index*5 Addressing Arithmetic Deduplication (refactor) | `08-refactoring` | **IN PROGRESS** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | Filed from `BL-0173` (run #281, systematic duplicate-sequence scan). Extracts the index*5 addressing arithmetic duplicated across `czt_region_hl`/`dsr_p`/`setup_zone_collects`/`gw_neighbor_hl`/`ilmc_evict` into `idx5_to_hl`. Expected modest net ROM decrease. |
