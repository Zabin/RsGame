...
| [IP-8010](packages/IP-8010-point-in-box-hittest-deduplication.md) | Point-in-Box Hit-Test Deduplication (refactor) | `08-refactoring` | **READY** | none | **AUTHORIZED (G3, user "Authorized all refactoring," 2026-07-20)** | Filed from `BL-0170` (run #275, own tree survey). Extracts the point-in-box test duplicated between `check_collisions` and `inf_mob_contact_check` into one shared subroutine, `pib_reg_minus_origin`. **Scope corrected during planning**: `inf_projectile_hittest`'s own inlined test computes the operands in the reverse order (a different predicate, not a duplicate) — deliberately left unmodified; extracting a single-use routine would cost bytes, not save them. Expected small net ROM decrease against the tranche's current 98-byte headroom. No requirement/architecture/WRAM impact; no new tests (existing `T8`/`T36` coverage is the acceptance bar). |
