...
> `IP-1080` is now `READY` (its hard `VERIFIED` prerequisite is satisfied) but remains
> unauthorized — no G3 decision on record. `FS-108`'s rendering half remains unplanned (its own
...
| [IP-1080](packages/IP-1080-maze-aware-edge-classification.md) | Maze-aware transition-edge classification, logic half (FS-108 / FEAT-2100) | `08-code-implementation` | **READY** | IP-1070 (**VERIFIED**), IP-1030 (VERIFIED) | **NOT AUTHORIZED — no G3 on record** | Planned 2026-07-11. Render-time open/blocked/absent classification inside `draw_region_arrows`'s existing per-direction loop, reusing `check_zone_transition`'s own grid-boundary arithmetic pattern; the blocked case is a logic-only no-op render-wise in this package (no new WRAM, no new tile). Covers `FR-2330` **partially** — the rendering half (tile art/palette) is not planned by this package, still blocked on `BL-0068`'s unrouted `GDS-08` delta; FS-108's own Acceptance Criterion 4 stays explicitly open in this package's Definition of Done, not silently implied covered. New suite **T20** planned (open/blocked/absent classification checks, sharing `IP-1070`'s T19 corpus). **`IP-1070` reached `VERIFIED` 2026-07-12 ([VR-1070](verification/VR-1070-maze-shaped-region-adjacency.md))** — this package's hard prerequisite is now satisfied; unblocked to `READY`. Still **not authorized** — no G3 decision on record; requires an explicit user go-ahead before `08-code-implementation` may execute it. |
...
- **Authorization state: `IP-1070` authorized 2026-07-11** (explicit user G3, `BL-0069` —
  ...
  **`IP-1080` is now `READY`** (its `VERIFIED` prerequisite is satisfied) **but remains
  unauthorized** — a separate G3 question the user has not yet answered.
...
graph TD
    ...
    IP1080["IP-1080 maze-aware edge<br/>classification, logic half<br/>(READY, not authorized)"]
    IP1020 --> IP1070
    IP1070 --> IP1080
    IP1030 --> IP1080
...
- **Authorization state: `IP-1070` authorized 2026-07-11** (explicit user G3, `BL-0069`) —
  ...
  unauthorized — a separate G3 decision, not yet asked of the user.
