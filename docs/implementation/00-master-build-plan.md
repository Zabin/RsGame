...
| [IP-1106](packages/IP-1106-infinite-mode-nine-identity-value-widening.md) | Infinite Mode nine-identity value-range widening | `08-code-implementation` | **BLOCKED** (one real prerequisite, `IP-1022`) | IP-1105 (VERIFIED), IP-1022 (READY), IP-1033 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `materialize_region`'s draw `%5`→`%9`, extends `inf_treasure_pos` to nine entries. The deepest point in this delta's own dependency chain (`IP-1033`/`IP-1105` → `IP-1022` → `IP-1106`) — shipping ahead of its own prerequisites would reintroduce the exact live-regression risk `IP-1105`'s own TWBS entry first identified. `IP-1105`/`IP-1033` both `VERIFIED`; `IP-1022` (now `READY`, not yet implemented) remains the real blocker. |

**Critical-path chain within this delta:** `IP-1033`/`IP-1105` (parallel-eligible roots, both now
`VERIFIED`) → `IP-1022` (`READY`) → `IP-1106` (`BLOCKED` on `IP-1022`). Does not extend or block
any other in-flight critical path — both Infinite Mode's own tranche and the finite-mode procgen
