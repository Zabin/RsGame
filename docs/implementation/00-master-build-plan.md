...
| [IP-1022](packages/IP-1022-finite-mode-nine-identity-generation-and-dispatch.md) | Finite-mode nine-identity generation & screen dispatch | `08-code-implementation` | **BLOCKED** (ROM-budget overflow, see Blocking Report below) | FR-4310/FR-4320 (baselined), IP-1033 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `generate_world`'s clamp `[0,4]`→`[0,8]`, extends `dsr_p_dispatch`'s cascade to all nine identities, splices `IP-1033`'s staged content into `ZONE_COLLECTS`'s real array. `CR-08`'s resolution unblocked this package's own planning; `IP-1033` `VERIFIED` (2026-07-17). **`08-code-implementation` attempted 2026-07-17**: all code-side changes (worldgen.py/asm_game.py clamp+dispatch widening, `ALL_SCREENS`/`ZONE_COLLECTS` extension, `build_rom.py` patch wiring) implemented mechanically exactly per the package's own spec — but adding `village`/`cave`/`desert`/`plains` to `ALL_SCREENS` means `build_rom.py` now emits their full 576-tile + 576-attr arrays (1,152 bytes each, 4,608 bytes total) into the ROM for the first time (`IP-1033`'s own content stayed inert/unwired, so this is the first package to actually cost this budget). Only 1,406 bytes of headroom exist (31362/32768 used) — a **3,202-byte shortfall**, causing `build_rom.py` to crash (`IndexError: bytearray index out of range`) attempting to emit past the 32768-byte ceiling. **This package's own Risks section (§13) never named ROM budget as a risk** — a real planning gap, not a code defect. All source edits reverted (tree confirmed back to the last-known-good, buildable, 309/309-passing state — verified by rebuild + full suite run after revert); no partial/broken state left in the tree. Blocking Report filed; recommended owner `07-implementation-planning` (or `03-architecture-design-synthesis` if a genuine bank-switching/MBC decision is needed, per `ADR-0001`'s own named C7 future-supersession trigger) to resolve the shortfall — e.g. trim/compress other ROM data, revisit whether all nine screens need distinct full-grid storage, or authorize a multi-bank ROM. `IP-1106` remains `BLOCKED` on this package, unchanged. |
| [IP-1106](packages/IP-1106-infinite-mode-nine-identity-value-widening.md) | Infinite Mode nine-identity value-range widening | `08-code-implementation` | **BLOCKED** (one real prerequisite, `IP-1022`) | IP-1105 (VERIFIED), IP-1022 (BLOCKED — ROM budget), IP-1033 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `materialize_region`'s draw `%5`→`%9`, extends `inf_treasure_pos` to nine entries. The deepest point in this delta's own dependency chain (`IP-1033`/`IP-1105` → `IP-1022` → `IP-1106`) — shipping ahead of its own prerequisites would reintroduce the exact live-regression risk `IP-1105`'s own TWBS entry first identified. `IP-1105`/`IP-1033` both `VERIFIED`; `IP-1022` now `BLOCKED` on a genuine ROM-budget overflow (`BL-0134`), not merely unimplemented. |
| [IP-9150](packages/IP-9150-tile-data-padding-trim.md) | Tile-data padding trim (ROM-budget hygiene, `BL-0134`'s safe mechanical recovery) | `08-code-implementation` | **NOT STARTED** | None | **Not yet authorized — new package, awaits explicit G3 go-ahead (not covered by "Build all six," which named only the four `BL-0128`/`BL-0127` packages above)** | `tiles.py`'s `build_tile_data()` allocates a fixed 256-tile (4,096-byte) array; only 182 slots (indices 0–181, highest is `TL_TORCH=0xB5`) are ever used. Trimming to 184 tiles (2,944 bytes) + correspondingly reducing the boot-time tile-copy DMA count (`asm_game.py:359`) recovers 1,152 bytes with zero behavior change. Independent of `IP-1022` — safe to ship regardless of how `BL-0134`'s larger residual shortfall is resolved. Does **not** by itself unblock `IP-1022` (recovers 1,152 of ~3,358 bytes needed; ~2,200-byte residual gap remains). |

**Critical-path chain within this delta:** `IP-1033`/`IP-1105` (parallel-eligible roots, both now
`VERIFIED`) → `IP-1022` (`BLOCKED` — ROM-budget overflow, `BL-0134`) → `IP-1106` (`BLOCKED` on
`IP-1022`). `IP-9150` is a parallel, independent hygiene package — not on this critical path, but
a prerequisite step toward eventually closing `IP-1022`'s own gap. Does not extend or block any
other in-flight critical path — both Infinite Mode's own tranche and the finite-mode procgen
tranche are already `VERIFIED` and closed independent of this delta.

**`IP-1022`'s residual ROM-budget shortfall (~2,200 bytes after `IP-9150` lands) requires a user
decision before any further package can be authored against it** — see
[`01-technical-work-breakdown.md`](01-technical-work-breakdown.md)'s `BL-0134` entry for the full
byte math and the named options (descope this pass; runtime tile-array compression, a new engine
capability; or reversing `ADR-0001`'s single-bank decision per its own named `C7` trigger). Not a
G3 authorization question — a genuine architecture-direction question, routed `NEEDS-USER`.
