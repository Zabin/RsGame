...
| [IP-1129](packages/IP-1129-infinite-mode-combat-weapon-tier-funding.md) | Infinite Mode Combat: Weapon-Tier Funding Economy | `08-code-implementation` | **READY** | IP-1122 (VERIFIED), IP-1103 (VERIFIED) | **AUTHORIZED (G3, user go-ahead, 2026-07-19 — "Authorize both")** | Planned 2026-07-19 (`07-implementation-planning`). New `inf_tier_spend`, mirroring `inf_heal_spend`'s own exact structure — decrements `RUNNING_TREASURE_COUNT`, increases `WEAPON_TIER` capped at 3, **spending even at the cap** (matches `FR-11500`'s own real shipped precedent, `T31.d2` — not a no-op, a correction made during this same planning pass after an initial draft got it backwards). No new WRAM. New suite `T38` (5 checks). Not yet player-reachable (`BL-0148`-class input-binding gap, named not resolved). Both dependencies `VERIFIED` — `READY`, not authorized. |
