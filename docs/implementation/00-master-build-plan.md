...
| [IP-8050](packages/IP-8050-keyitem-flags-addressing-deduplication.md) | KEYITEM_FLAGS Addressing Deduplication (refactor) | `08-refactoring` | **IN PROGRESS** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | Filed from `BL-0175` (run #282, systematic scan). Extracts the index-to-`KEYITEM_FLAGS`-address computation duplicated across six sites into `keyitem_flags_hl`. Expected net ROM decrease (~16 bytes est.). |
