...
| [IP-8020](packages/IP-8020-abs-delta-from-player-deduplication.md) | Absolute-Delta-From-Player Deduplication (refactor) | `08-refactoring` | **NOT STARTED** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization for this session's further well-scoped refactor work)** | Filed from `BL-0171` (run #279, continued tree survey). Extracts the "absolute delta from `PLAYER_X`/`PLAYER_Y`" computation duplicated between `inf_mob_move` and the knockback block in `inf_mob_contact_check` into one shared subroutine, `abs_delta_from_player`. Expected net ROM decrease (larger than `IP-8010`'s, bigger duplicated block). No requirement/architecture/WRAM impact; no new tests. |
