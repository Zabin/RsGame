...
| [IP-8030](packages/IP-8030-treasure-spend-gate-deduplication.md) | Treasure-Spend Gate-and-Decrement Deduplication (refactor) | `08-refactoring` | **IN PROGRESS** | none | **AUTHORIZED (G3, user "continue iterating the refactoring... don't stop to ask," 2026-07-20 — blanket authorization)** | Filed from `BL-0172` (run #280, continued tree survey). Extracts the gate-and-decrement prefix duplicated between `inf_heal_spend`/`inf_tier_spend` into `treasure_spend_gate_and_decrement`. Expected net ROM decrease. |
