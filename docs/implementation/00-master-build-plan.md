...
> logic half, `ADR-0012`) — **`IP-1070` authorized 2026-07-11 (user G3, `BL-0069`), now `READY`.**
> `IP-1080` remains unauthorized and `BLOCKED` on `IP-1070` reaching `VERIFIED`. `FS-108`'s
> rendering half remains unplanned, riding `BL-0068`'s still-open `GDS-08` delta. Owned by
...
- **Authorization state: `IP-1070` authorized 2026-07-11** (explicit user G3, `BL-0069` —
  "Authorize IP-1070"), now `READY`. **`IP-1080` remains unauthorized** — a separate G3 question
  once it becomes eligible (`IP-1070` reaching `VERIFIED`). Neither `BL-0064`/`BL-0065`/`BL-0067`
  fell under the `BL-0001`…`BL-0005` G3 bootstrap carve-out — the same category
  `IP-9050`/`IP-9060`/`IP-9070` fell into, requiring the explicit `BL-0062` authorization those
  three packages needed.
...
- **`IP-1070` is now `READY`** (fully specified — `FS-107` has no blocking Open Questions, all
  three resolved by this package's own §6 — and its sole dependency `IP-1020` is `VERIFIED`).
  `IP-1080` remains `BLOCKED` on `IP-1070` reaching `VERIFIED`.
- **Authorization state: `IP-1070` authorized 2026-07-11** (explicit user G3, `BL-0069`).
  `IP-1080` remains unauthorized — not yet eligible to ask about (still `BLOCKED`, not `READY`).
