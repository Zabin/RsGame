...
| [IP-1121](packages/IP-1121-infinite-mode-combat-mob-materialization-and-rendering.md) | Infinite Mode Combat: Mob Materialization, Rendering & Defeat | `08-code-implementation` | **READY** | IP-1101/IP-1102 (VERIFIED), IP-1125 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | Defines `COMBAT_MODE`/`MOB_COUNT`/`MOB_DATA` (`0xC6B5`–`0xC6D4`, boot-cleared); extends `inf_ensure_window` (per-region mob-presence draw) and `update_oam` (mob rendering). Delta's dependency root — all dependencies now `VERIFIED`. |
