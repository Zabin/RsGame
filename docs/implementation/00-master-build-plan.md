...
| [IP-1111](packages/IP-1111-biome-family-sub-theme-playback-selection.md) | Biome-Family Sub-Theme Playback Selection | `08-code-implementation` | **READY** | IP-1110 (VERIFIED), IP-1022 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Hooks into `do_screen_redraw`'s per-state dispatch (default reset to main theme) and `dsr_p_dispatch`'s per-identity cascade (override to matching sub-theme) — resolves `FS-111`'s Open Question 1. Also fixes `music_tick`'s hardcoded loop-restart target (new `MUSIC_BASE_LO`/`MUSIC_BASE_HI`), a correctness gap this planning pass found, not named in `FS-111`. **Re-planned 2026-07-17** (`07-implementation-planning`, the touch `IP-1110`'s Outstanding Issue requested): §5/§6 now consume `IP-1110`'s shipped `music_table` (biome-id-indexed, `zc_table` precedent) via one shared `music_select` subroutine + a single `CALL` at `dsr_p_dispatch`'s entry (A = biome-id there on both mode paths, `IP-1022` `VERIFIED`) — replacing the original eight per-branch snippets; new `music_tbl` patch key; `mus_reset` still retired per the `music_tick` loop-restart fix. All §6 citations re-derived against the live tree. **`READY`** — both dependencies `VERIFIED`, authorization on record. |
