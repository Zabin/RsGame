...
> zone-transition threshold fix planned 2026-07-12`** (`IP-9120`, `BL-0076`) — a Critical
> regression from `IP-9090`'s own commit: the corrected RIGHT movement clamp (max `X=152`) fell
> below `check_zone_transition`'s own RIGHT-edge trigger (`X>=156`), breaking every rightward
> zone transition in every generated world. `READY`, **not yet authorized** — a G3 ask is
> pending, recommended for expedited handling given the severity. Owned by
...
| [IP-9120](packages/IP-9120-right-zone-transition-threshold-fix.md) | RIGHT zone-transition threshold fix (BL-0076) | `08-code-implementation` | **READY** | IP-1010 (VERIFIED, `check_zone_transition`'s own bootstrap dependency), IP-9050 (COMPLETE, the routine's own current shape) | **NOT AUTHORIZED — no G3 on record** | Planned 2026-07-12. **Critical regression**: `IP-9090` (this session) correctly lowered the RIGHT movement clamp's max reachable `PLAYER_X` from `159` to `152` (`BL-0052`), but `check_zone_transition`'s own RIGHT-edge trigger still requires `PLAYER_X >= 156` — the two thresholds used to overlap by coincidence under the old, buggy clamp, and no longer do, so **no player can transition rightward into a new zone through normal gameplay input, at all, in any generated world**, as of `IP-9090`'s commit. Confirmed via real, sustained button-press input (not memory-forced teleportation). Fix: `check_zone_transition`'s RIGHT-edge comparison `CP_n(156)`→`CP_n(152)`, mirroring DOWN's already-consistent threshold/clamp-ceiling match. Supersession sweep confirmed clean — exactly one other call site shares the old constant class, and it's `IP-9090`'s own already-correct clamp. New check `T7.11` (real button-press-driven positive-transition test — the exact class of test missing across all four directions that let this regression ship undetected). Given Critical severity, an expedited G3 ask is recommended over queuing behind lower-severity work. |
...
graph TD
    ...
    IP9120["IP-9120 RIGHT zone-transition<br/>threshold fix<br/>(READY, not authorized)"]
    IP1010 --> IP9120
    IP9050 --> IP9120

    style IP9120 fill:#f9c,stroke:#333,stroke-width:2px
```
...
### RIGHT zone-transition threshold fix (planned 2026-07-12)
...
- **`IP-9120`** (`BL-0076`): a direct regression from this session's own `IP-9090`. `IP-9090`
  ...
  (`BL-0052`), but `check_zone_transition`'s own RIGHT-edge trigger still requires
  `PLAYER_X >= 156` — the two thresholds overlapped by coincidence under the old, buggy clamp and
  no longer do, so no player can transition rightward into a new zone through normal gameplay
  input, at all, in any generated world, as of `IP-9090`'s commit. Root cause and fix (a single
  comparison-operand change, `CP_n(156)`→`CP_n(152)`) fully diagnosed at intake, confirmed via
  real, sustained button-press input (not memory-forced teleportation — the discipline gap that
  let this regression ship undetected in the first place). `READY`, not yet implemented.
- **Authorization state: NOT AUTHORIZED — no G3 on record.** `BL-0076` is not covered by the
  `BL-0001`…`BL-0005` bootstrap carve-out. Given Critical severity (breaks core navigation
  universally, in the currently pushed build, for every player), an expedited G3 ask is
  recommended over letting this queue behind lower-severity open work.
