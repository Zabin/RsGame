...
> logic half, `ADR-0012`) — an `08-code-implementation` attempt hit a Blocking Report (the
...
> carve+braid draws, `gw_prng_step` itself untouched) — **`IP-1070` back to `READY`, authorized
> (user G3, `BL-0069`, unaffected throughout).** `IP-1080` remains unauthorized and `BLOCKED` on
> `IP-1070` reaching `VERIFIED`. `FS-108`'s rendering half remains unplanned, riding `BL-0068`'s
> still-open `GDS-08` delta. Owned by
...
| [IP-1070](packages/IP-1070-maze-shaped-region-adjacency.md) | Maze-shaped region adjacency (FS-107 / FEAT-9100) | `08-code-implementation` | **READY** | IP-1020 (VERIFIED) | **YES — explicit user G3, 2026-07-11 (BL-0069)** | **Re-planned 2026-07-11 (light touch, `ADR-0013`).** An `08-code-implementation` attempt found the carve phase directly correct but hit a Blocking Report: the shipped `gw_prng_step` collapses to a degenerate fixed point/short cycle under the braid pass's many back-to-back draws (root-caused by `R113`). `ADR-0013` decides the fix — a loop-local, never-persisted counter XORed into each PRNG draw at both the carve phase's direction-draw site and the braid phase's keep/prune-draw site, never fed back into `gw_prng_step`'s own carried state; `gw_prng_step` itself and its only other caller (biome assignment) stay untouched. Package document §4/§6/§7/§13 updated with the perturbation detail (`worldgen.py`'s oracle mirror must replicate it step-for-step, same lockstep discipline as the carve/braid algorithm itself); §13 also records `ADR-0013`'s own explicit deferral of `gw_prng_step`'s underlying repair as a known, accepted limitation. Sanity-checked: no leftover code from the earlier attempt (`asm_game.py` confirmed clean, already reverted). No redesign — the carve/braid algorithm itself (§6 tasks 3–4) is unchanged. Back to `READY`; authorization `BL-0069` stands, unaffected throughout. |
...
- **Authorization state: `IP-1070` authorized 2026-07-11** (explicit user G3, `BL-0069` —
  "Authorize IP-1070"). **Back to `READY`** — an `08-code-implementation` attempt hit a Blocking
  Report (the existing PRNG doesn't stay well-distributed across the braid pass's many
  consecutive draws), root-caused by `R113` and resolved by `ADR-0013` (see the package status
  table's own `IP-1070` row) with a light package-document touch, not a redesign. Authorization
  stood throughout — the blocker was a dependency defect, not a missing go-ahead. **`IP-1080`
  remains unauthorized** — a separate G3 question once it becomes eligible.
...
graph TD
    ...
    IP1070["IP-1070 maze-shaped<br/>region adjacency<br/>(READY, authorized)"]
...
- **`IP-1070` is back to `READY`** — an `08-code-implementation` attempt found the design's own
  dependency on the existing PRNG's draw-quality didn't hold (a Blocking Report); `ADR-0013`
  resolved it with a scoped counter-XOR perturbation, and this pass re-planned the package with a
  light touch (§4/§6/§7/§13), no redesign — see the package status table's own `IP-1070` row.
  `IP-1080` remains `BLOCKED` on `IP-1070` reaching `VERIFIED`.
- **Authorization state: `IP-1070` authorized 2026-07-11** (explicit user G3, `BL-0069`) —
