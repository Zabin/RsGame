...
| [IP-1022](packages/IP-1022-finite-mode-nine-identity-generation-and-dispatch.md) | Finite-mode nine-identity generation & screen dispatch | `08-code-implementation` | **READY** (re-planned against `ADR-0020`) | FR-4310/FR-4320 (baselined), IP-1033 (VERIFIED), ADR-0020 (accepted) | **AUTHORIZED (G3, user "Build all six," 2026-07-16 — re-plan is within the same authorized scope, not new work)** | Widens `generate_world`'s clamp `[0,4]`→`[0,8]`, extends `dsr_p_dispatch`'s cascade to all nine identities, splices `IP-1033`'s staged content into `ZONE_COLLECTS`'s real array. First attempt (2026-07-17) hit a ~3,358-byte ROM-budget overflow baking the four new screens' full arrays (`BL-0134`) — reverted cleanly. **Re-planned 2026-07-17 against [ADR-0020](../architecture/adr/ADR-0020-procedural-screen-fill-for-rom-budget.md)**: the four new screens now render via a runtime procedural-fill routine (matching each screen's own Python modulo formula) + a compact landmark-overlay list, instead of baked arrays — estimated ~4,272-byte recovery, comfortably closing the gap without bank switching. New Verification Checklist obligation: an oracle-parity check proving the on-device fill+overlay output is byte-for-byte identical to each Python `*_screen()` function. `IP-1106` remains `BLOCKED` on this package until it ships and verifies. |
| [IP-1106](packages/IP-1106-infinite-mode-nine-identity-value-widening.md) | Infinite Mode nine-identity value-range widening | `08-code-implementation` | **BLOCKED** (one real prerequisite, `IP-1022`) | IP-1105 (VERIFIED), IP-1022 (READY), IP-1033 (VERIFIED) | **AUTHORIZED (G3, user "Build all six," 2026-07-16)** | Widens `materialize_region`'s draw `%5`→`%9`, extends `inf_treasure_pos` to nine entries. The deepest point in this delta's own dependency chain (`IP-1033`/`IP-1105` → `IP-1022` → `IP-1106`) — shipping ahead of its own prerequisites would reintroduce the exact live-regression risk `IP-1105`'s own TWBS entry first identified. `IP-1105`/`IP-1033` both `VERIFIED`; `IP-1022` now `READY` (re-planned against `ADR-0020`, not yet implemented) remains the real blocker. |
...
`VERIFIED`) → `IP-1022` (`READY`, re-planned against `ADR-0020`) → `IP-1106` (`BLOCKED` on
`IP-1022`). `IP-9150` is a parallel, independent hygiene package, not on this critical path and no
longer a prerequisite for it. Does not extend or block any other in-flight critical path — both
Infinite Mode's own tranche and the finite-mode procgen tranche are already `VERIFIED` and closed
independent of this delta.

**`IP-1022`'s ROM-budget gap is resolved at the planning level.** Per the user's own direction
("pursue being as efficient as possible with the ROM size... implement [compression] without
asking... If there is a genuine need for bank switching then implement that too"),
[`ADR-0020`](../architecture/adr/ADR-0020-procedural-screen-fill-for-rom-budget.md) decided a
runtime procedural-fill + landmark-overlay technique (estimated ~4,272-byte recovery, comfortably
exceeding the ~3,358-byte shortfall) — `IP-1022` has been re-planned against it and is `READY`.
`ADR-0011`'s bank switching remains unimplemented and uncommitted-to for this delta; it was not
"genuinely needed" once the cheaper, already-precedented option closed the gap on its own. No
further user gate applies — `IP-1022`'s re-plan is within its own existing "Build all six"
authorization.
