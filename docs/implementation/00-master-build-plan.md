...
| [IP-1120](packages/IP-1120-infinite-mode-combat-mode-gating.md) | Infinite Mode Combat: Mode Gating & UI | `08-code-implementation` | **BLOCKED** (ROM budget — see 2026-07-18 Blocking Report below) | IP-1121 (VERIFIED), IP-1100 (VERIFIED) | **AUTHORIZED (G3, user "Yes, build all six," 2026-07-17)** | Re-planned in full 2026-07-17 against `GDS-01` §4e (`BL-0146` closed): new `GS_COMBAT_MODE_CONFIRM=12` state (binary Y/N, `CMC_CURSOR` WRAM byte `0xC6DD`), reached only after `MODE SELECT`'s "infinite" confirm; `ms_infinite`'s existing transition retargeted (one constant) — `MODE SELECT`'s own two options and the finite path completely unaffected. New `combat_mode_confirm_screen()` (font-only, zero new tile art). **Blocking Report, 2026-07-18 (`08-code-implementation`):** implemented as specified (code+content, mirroring `T33`'s own suite in full) and confirmed structurally correct, but a full build overflows the 32768-byte ROM by **542 bytes** (measured directly: `combat_mode_confirm_screen()`'s own registration in `ALL_SCREENS` costs the fixed per-screen 1,152-byte tile+attr array — `zero new tile art` does not mean zero array cost, `build_rom.py`'s own screen loop emits a full 576+576-byte array per `ALL_SCREENS` entry regardless of content novelty, mirroring `BL-0134`'s own exact precedent for `IP-1022`'s four new finite-mode screens — against only 866 bytes of headroom remaining after `IP-1122`/`IP-1123` landed, plus this package's own new code). All source edits reverted; tree confirmed back to the `IP-1123`-committed state (rebuild 31902/32768, full suite 349/349) — no broken/partial state left behind. Two mechanical checks already ruled out: `TILE_DATA_TILES`'s own trim slack (`IP-9150`'s own precedent) is only ~48 bytes, nowhere near enough alone; this screen's content is plain menu text, not a candidate for `ADR-0020`'s own procedural-fill technique (that pattern fits repeating terrain formulas, not four lines of static UI text). Routed to `07-implementation-planning` to find either a mechanical recovery elsewhere in the budget, or (mirroring `IP-1022`'s own precedent) a design change — e.g. reusing `mode_select_screen`'s own already-registered tile/attr array as this state's base and drawing this screen's differing text (title + NO/YES labels) at runtime instead of baking a second full array, the same "static base + runtime overlay" technique `draw_sse_digits`/`draw_ise_digits` already use for their own dynamic content. |
