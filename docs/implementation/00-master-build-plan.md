...
| [IP-9160](packages/IP-9160-procedural-screen-zone-name-restoration.md) | Procedural-screen zone-name restoration (`BL-0138`, content-review Medium) | `08-content-authoring` | **NOT STARTED** | IP-1022 (VERIFIED), IP-1033 (VERIFIED) | **Not yet authorized — new remediation package, awaits explicit G3 go-ahead** | The four `ADR-0020` procedural screens never write the row-0 zone name their Python oracles ship — the previous screen's name persists ([content-review finding 1](../reviews/content-review-nine-biome-family-delta.md)). Fix is pure data: append each screen's name-region row-0 cells (mechanically derived from its own `*_screen()` oracle) to the existing `*_LANDMARKS` lists — `apply_landmark_overlay` already writes row 0 unfiltered, `build_rom.py`'s emission is generic (both direct-read confirmed). Tests: `T13.e` exclusion narrowed to the live digit cells (row 0's static region enters the parity check), new `T13.g` stale-name regression both directions. ≈128 data bytes vs. 354 headroom. |
