...
| R115 | [Combat sub-mode hardware feasibility](encyclopedia/R115-combat-submode-hardware-feasibility.md) | projectile movement/hit-detection cost per frame (no hardware collision detection on GBC); concurrent-sprite OAM budget (9/40 entries used today, 31 free); APU channel headroom (channels 2–4 entirely unused) for combat mode SFX (filed via `BL-0133`/`BL-0145`) | ✅ |
...
| R212 | [Wordless environmental storytelling & biome-adjacency grammar](encyclopedia/R212-wordless-environmental-storytelling-biome-grammar.md) | narrative-through-world-structure technique; the adjacency-grammar rule set grounding MSTR-001 C9 (filed via `BL-0030`/D2); **v1.1 delta (2026-07-16)** extends the grammar axis to nine identities, grounding `CR-08` (`BL-0128`) | ✅ |
...
| R217 | [Procedural/generative music composition: main theme + biome sub-themes](encyclopedia/R217-procedural-generative-music-composition.md) | leitmotif/theme-and-variation technique for a main-theme-plus-sub-themes structure; Markov-chain and transform-based procedural generation feasibility on constrained hardware; build-time-vs-runtime split mirroring `worldgen.py`'s own precedent (filed via `BL-0127`) | ✅ |
| R218 | [Combat & enemy design for an opt-in, tonally-distinct sub-mode](encyclopedia/R218-combat-enemy-design-opt-in-submode.md) | non-graphic "poof" defeat convention (Zelda precedent); heart-container health HUD (reuses this project's existing `TL_HEART_FULL`/`TL_HEART_EMPTY` tiles at zero new art cost); difficulty-gated optional content precedent (Double Dragon II, TimeSplitters 2) grounding `MSTR-001` C11's "opt-in, explicitly gated" framing; grounds `ADS-002`'s fuller architecture pass (filed via `BL-0133`/`BL-0145`) | ✅ |
| R219 | [Ranged-weapon upgrade & progression conventions](encyclopedia/R219-ranged-weapon-upgrade-progression.md) | run-and-gun tiered-power-up conventions (Contra); currency-spent vs. pickup-based vs. score-gated acquisition; death-based downgrade vs. persistent power, grounding a `WEAPON_TIER` funding mechanism compatible with this project's own "treasure spent, no ammo/durability" constraints (filed via `BL-0155`) | ✅ |
| R220 | [Movement-based multi-directional weapon aiming conventions](encyclopedia/R220-movement-based-directional-aiming.md) | 4-way/8-way/free-aim discrete-direction schemes; last-pressed-direction vs. held-direction-vs-facing semantics for deriving fire direction without a dedicated aim input, grounding a future widening of `PLAYER_DIR`'s own 2-state shape (filed via `BL-0157`); **`WebFetch` blocked this session (egress policy) — both citations marked needs-fetch-verification** | ✅ |
